---
title: How to install a Linux master target server for failover from Azure to on-premises| Azure
...
1. On the process server, go to **C:\Program Files (x86)\Azure Site Recovery\home\svsystems\pushinstallsvc\repository**.
...
1. Copy the passphrase from **C:\ProgramData\Azure Site Recovery\private\connection.passphrase** on the configuration server. Then save it as **passphrase.txt** in the same local directory by running the following command:
