...
* The primary datacenter is in Singapore (Azure South-China East). The disaster recovery datacenter is in Hong Kong (Azure China East). In this scenario, local high availability is provided by two VMs that run SQL Server AlwaysOn in synchronous mode in Singapore.
