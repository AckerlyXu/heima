---
title: Review the architecture for Hyper-V replication (with System Center VMM) to Azure with Azure Site Recovery | Azure
...
--- | --- | ---
**Azure** | In Azure you need a Azure account, an Azure storage account, and a Azure network. | Replicated data is stored in the storage account, and Azure VMs are created with the replicated data when failover from your on-premises site occurs.<br/><br/> The Azure VMs connect to the Azure virtual network when they're created.
...
<!--Update_Description: new articles on site recovery vmm to azure walkthrought architecture -->