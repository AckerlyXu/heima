---
title: Azure Traffic Manager with Azure Site Recovery | Azure
...
- Utilizing the **Priority** routing method, **Company C** creates two endpoints - **Primary** for the source region (Azure China East) and **Failover** for the recovery region (Azure China North). **Primary** is assigned Priority 1 and **Failover** is assigned Priority 2.
...
For example, if the endpoint in Germany Central fails, the application can quickly be recovered to Germany Northeast. The new endpoint handles traffic originating from Germany with minimal downtime for users. Similarly an endpoint outage in China North can be handled by recovering the application workload to China North, with Azure Traffic Manager handling DNS redirects to the available endpoint.
