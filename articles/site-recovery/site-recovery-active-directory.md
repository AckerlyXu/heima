...
Follow the instructions to [create a domain controller in an Azure virtual network](/documentation/articles/active-directory-install-replica-active-directory-domain-controller/). When you  promote the server to a domain controller role specify the same domain name that's used on the primary site.

Then [reconfigure the DNS server for the virtual network](/documentation/articles/active-directory-install-replica-active-directory-domain-controller/#reconfigure-dns-server-for-the-virtual-network), to use the DNS server in Azure.
...
1. [Replicate](/documentation/articles/site-recovery-replicate-vmware-to-azure/) the domain controller/DNS virtual machine using Site Recovery.
...
1. If you're replicating to another on-premises site and you're using DHCP follow the instructions to [setup DNS and DHCP for test failover](/documentation/articles/site-recovery-test-failover-vmm-to-vmm/#prepare-dhcp)
