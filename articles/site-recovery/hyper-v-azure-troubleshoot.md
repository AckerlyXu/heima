---
title: Troubleshoot Hyper-V to Azure replication with Azure Site Recovery | Azure
...
3. Check that required services are running. If they aren't, restart them.
    - If you're replicating Hyper-V without VMM, check that these services are running on the Hyper-V host:
        ...
        - Azure Recovery Services Agent service
        - Azure Site Recovery service
        - WMI Provider Host service
    - If you're replicating with VMM in the environment, check that these services are running:
        - On the Hyper-V host, check that the Virtual Machine Management service, the Azure Recovery Services Agent, and the WMI Provider Host service are running.
...
7. Check if the VM is experiencing a high churn rate:
    ...
    - In addition you can [verify storage scalability targets](/storage/common/storage-scalability-targets.md#scalability-targets-for-a-storage-account).
...
**Applications and Service Logs/MicrosoftAzureRecoveryServices/Replication** (Hyper-V host) | Logs to troubleshoot Azure Recovery Services Agent issues. 
**Applications and Service Logs/Microsoft/Azure Site Recovery/Provider/Operational** (Hyper-V host)| Logs to troubleshoot Azure Site Recovery Service issues.
