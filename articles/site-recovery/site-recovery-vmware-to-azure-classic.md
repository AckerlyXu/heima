<properties
    pageTitle="Replicate VMware VMs and physical servers to Azure in the classic portal | Azure"
...
This article includes contains instructions for an enhanced deployment in the classic Azure portal. We recommend you use this version for all new deployments. If you've already deployed using the earlier legacy version we recommend that you migrate to the new version. Read [more](/documentation/articles/site-recovery-vmware-to-azure-classic-legacy/#migrate-to-the-enhanced-deployment) about migration.
...
--- | ---
**Azure account**| You'll need a [Microsoft Azure](https://azure.cn/) account. You can start with a [1rmb trial](/pricing/1rmb-trial/). [Learn more](/pricing/details/site-recovery/) about Site Recovery pricing. 
**Azure storage** | You'll need an Azure storage account to store replicated data. Replicated data is stored in Azure storage and Azure VMs are spun up when failover occurs. <br/><br/>You need a [standard geo-redundant storage account](/documentation/articles/storage-redundancy/#geo-redundant-storage). The account must be in the same region as the Site Recovery service, and be associated with the same subscription. Note that replication to premium storage accounts isn't currently supported and shouldn't be used.<br/><br/>We do not support the move of Storage accounts created using the new Azure portal across resource groups.[Read about](/documentation/articles/storage-introduction/) Azure storage.<br/><br/> 
...
4. In **Specify Target Resources** select the storage account you're using for replication and select whether the settings should be used for all workloads. Note that premium storage accounts aren't currently supported.
	...
	>[AZURE.NOTE] 1. We do not support the move of Storage accounts created using the [new Azure portal](/documentation/articles/storage-create-storage-account/) across resource groups.
...
3. You can modify these settings:
	...
	-  **Azure VM size**: The number of network adapters is dictated by the size you specify for the target virtual machine. [Read more](/documentation/articles/virtual-machines-linux-sizes/#size-tables) about sizes and adapters. Note that:
