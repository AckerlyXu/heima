...
> * [The Azure portal preview](site-recovery-vmware-to-azure.md)
...
1. Open the Azure Backup MMC snap-in on the main management server or on a management server running additional provisioned process servers. By default, a shortcut for Azure Backup is created on the desktop. Or, you can find it in C:\Program Files\Azure Recovery Services Agent\bin\wabadmin.
...
| --- | --- |
| **Azure account** |You need a [Azure](https://azure.microsoft.com/) account. You can start with a [free trial](https://azure.microsoft.com/pricing/free-trial/). For more information about Site Recovery pricing, see [Site Recovery](https://azure.microsoft.com/pricing/details/site-recovery/). |
| **Azure storage** |You need an Azure storage account to store replicated data. Replicated data is stored in Azure storage, and Azure VMs are spun up when failover occurs. <br/><br/>You need a [standard geo-redundant storage account](../storage/storage-redundancy.md#geo-redundant-storage). The account must be in the same region as the Site Recovery service and be associated with the same subscription. Replication to premium storage accounts isn't currently supported and shouldn't be used.<br/><br/>We do not support moving storage accounts created by using the [Azure portal preview](../storage/storage-create-storage-account.md) across resource groups. For more information, see [Introduction to Azure Storage](../storage/storage-introduction.md).<br/><br/> |
...
1. Sign in to the [Azure portal preview](https://manage.windowsazure.com/).
...
1. In the Azure portal preview, select **Create virtual network** and specify the network name, IP address range, and subnet name.
...
> [!WARNING]
> The Azure Recovery Service Agent proxy must be set up.
> After the installation is finished, start the Azure Recovery Services Shell from the Windows Start menu. In the command window that opens, run the following set of commands to set up the proxy server settings.
>
>
...
2. On the **Specify Protection Group Settings** page, specify a name for the group. In the **From** drop-down list, select the configuration server on which you want to create the group. **Target** is Azure.
...
The installers are available in C:\Program Files (x86)\Azure Site Recovery\home\svsystems\pushinstallsvc\repository.
...
4. In **Specify Target Resources**, select the storage account that you're using for replication and select whether the settings should be used for all workloads. Premium storage accounts aren't currently supported.
   ...
   > [!NOTE]
   > We do not support moving storage accounts created by using the [Azure portal preview](../storage/storage-create-storage-account.md) across resource groups.                           
...
4. After the failover finishes, you should also be able to see the replica Azure machine appear in **Virtual Machines** in the Azure portal preview. If you want to initiate an RDP connection to the Azure VM, you’ll need to open port 3389 on the VM endpoint.
...
4. Wait for the unplanned failover job to finish. You can monitor failover progress on the **Jobs** tab. Even if errors occur during unplanned failover, the recovery plan runs until it's complete. You should also be able to see the replica Azure machine appear in **Virtual Machines** in the Azure portal preview.
