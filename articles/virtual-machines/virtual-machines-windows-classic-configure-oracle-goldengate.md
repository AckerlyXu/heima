...
* You have created two Virtual Machines (VMs) in Azure using Oracle Enterprise Edition on Windows Server. Make sure that the Virtual Machines are in the [same cloud service](../load-balancer/load-balancer-overview.md) and in the same [Virtual Network](../virtual-network/index.md) to ensure they can access each other over the persistent private IP address.
