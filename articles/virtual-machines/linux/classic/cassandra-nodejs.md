...
Using the image created previously, you create the following virtual machines in the cloud server "hk-c-svc-north" and bind them to the respective subnets as shown below:
...
Log into one of the nodes (for example hk-c1-china-north) and run the following command to see the status of the cluster:
...
1. Using the Powershell command Get-AzureInternalLoadbalancer commandlet, obtain the IP address of the internal load balancer (for example  10.1.2.101). The syntax of the command is shown below:
...
2. Log into the web farm VM (for example hk-w1-china-north) using Putty or ssh
...
* Get-AzureInternalLoadbalancer -ServiceName "hk-c-svc-china-east"  
    ...
    Note the IP addresses (for example north - 10.1.2.101, east - 10.2.2.101) displayed.
