---
title: Use cloud-init to add a user to a Linux VM on Azure | Azure
...
To add a user to a Linux VM, create a file in your current shell named *cloud_init_add_user.txt* and paste the following configuration. You can use any editor you wish. Enter `sensible-editor cloud_init_add_user.txt` to create the file and see a list of available editors. Choose #1 to use the **nano** editor. Make sure that the whole cloud-init file is copied correctly, especially the first line.  You need to provide your own public key (such as the contents of *~/.ssh/id_rsa.pub*) for the value of `ssh-authorized-keys:` - it has been shortened here to simplify the example.
...
Now, create a VM with [az vm create](https://docs.azure.cn/zh-cn/cli/vm?view=azure-cli-latest#create) and specify the cloud-init file with `--custom-data cloud_init_add_user.txt` as follows:
...
<!-- Update_Description: new articles on using cloudinit to add users -->
<!-- ms.date: 01/08/2018 -->