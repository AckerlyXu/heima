---
title: Create a Windows VM from a specialized VHD in Azure | Azure
...
If you had a VM that you deleted and you want to reuse the OS disk to create a new VM, use [Get-AzureRmDisk](https://docs.microsoft.com/powershell/get-azurermdisk).
...
Create a managed disk from the specialized VHD in your storage account using [New-AzureRMDisk](https://docs.microsoft.com/powershell/module/azurerm.compute/new-azurermdisk). This example uses **myOSDisk1** for the disk name, puts the disk in *StandardLRS* storage, and uses *https://storageaccount.blob.core.chinacloudapi.cn/vhdcontainer/osdisk.vhd* as the URI for the source VHD.
...
You can take a snapshot of and entire VM (including all disks) or of just a single disk. The following steps show you how to take a snapshot of just the OS disk of your VM using the [New-AzureRmSnapshot](https://docs.microsoft.com/powershell/module/azurerm.compute/new-azurermsnapshot) cmdlet. 
...
Create a managed disk from the snapshot using [New-AzureRMDisk](https://docs.microsoft.com/powershell/module/azurerm.compute/new-azurermdisk). This example uses *myOSDisk* for the disk name.
...
Create the vNet. This example sets the virtual network name to be **myVnetName**, the location to **China North**, and the address prefix for the virtual network to **10.0.0.0/16**. 
...
Add the OS disk to the configuration using [Set-AzureRmVMOSDisk](https://docs.microsoft.com/powershell/module/azurerm.compute/set-azurermvmosdisk). This example sets the size of the disk to *128 GB* and attaches the managed disk as a *Windows* OS disk.
...
Create the VM using [New-AzureRMVM](https://docs.microsoft.com/powershell/module/azurerm.compute/new-azurermvm)the configurations that we just created.
