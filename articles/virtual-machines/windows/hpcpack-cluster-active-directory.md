...
5. After the app is added, select it in the **App registrations** list. Then click **Settings** > **Properties**. Enter the following information:
    ...
    * Change **App ID URI** to `https://<Directory_name>/<application_name>`. Replace `<Directory_name`> with the full name of your Azure AD tenant, for example, `hpclocal.partner.onmschina.cn`, and replace `<application_name>` with the name you chose previously.
