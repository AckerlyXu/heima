---
title: Handling maintenance notifications for Windows VMs in Azure | Azure
...
You can also use Azure Powershell to see when VMs are scheduled for maintenance. Planned maintenance information is available from the [Get-AzureRmVM](https://docs.microsoft.com/powershell/module/azurerm.compute/get-azurermvm) cmdlet when you use the `-status` parameter.
...
You can also get the maintenance status for all VMs in a resource group by using [Get-AzureRmVM](https://docs.microsoft.com/powershell/module/azurerm.compute/get-azurermvm) and not specifying a VM.
...
<!--Update_Description: new articles maintenance notifications for windows VM-->