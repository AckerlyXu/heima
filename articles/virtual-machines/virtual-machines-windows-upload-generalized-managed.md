...
<properties
    pageTitle="Create a managed Azure VM from a generalized on-premises VHDs | Azure"
...
You can create a new VM in Azure by uploading a VHD exported from an on-premises virtualization tool or from another cloud. Using [Managed Disks](/documentation/articles/storage-managed-disks-overview/) for the new VM simplifies the VM managment and provides better availability when the VM is placed in an availability set. If you are uploading a VHD that will be used to create multiple Azure VMs, you must first generalize VHD using [Sysprep](/documentation/articles/virtual-machines-windows-generalize-vhd/). Sysprep removes any machine-specific information and personal account information from the VHD. 

Azure managed disks removes the need of managing [storage accounts](/documentation/articles/storage-introduction/) for Azure VMs. You only have specify the type [Premium](/documentation/articles/storage-premium-storage-performance/) or [Standard](/documentation/articles/storage-standard-storage/) and size of disk you need, and Azure will create and manage the disk for you. 

> [AZURE.IMPORTANT]
> Review [Plan for the migration to Managed Disks](/documentation/articles/virtual-machines-windows-on-prem-to-azure/#plan-for-the-migration-to-managed-disks) before starting your migration to [Managed Disks](/documentation/articles/storage-managed-disks-overview/).
>
