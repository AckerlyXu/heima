<properties
    pageTitle="Capture an Azure Linux VM to use as a template | Azure"
...
Follow the steps in this article to generalize and capture your Azure Linux virtual machine (VM) in the Resource Manager deployment model. When you generalize the VM, you remove personal account information and prepare the VM to be used as an image. You then capture a generalized virtual hard disk (VHD) image for the OS, VHDs for attached data disks, and a [Resource Manager template](/documentation/articles/resource-group-overview/) for new VM deployments. This article details how to capture a VM image with the Azure CLI 1.0 for a VM using unmanaged disks. You can also [capture a VM using Azure Managed Disks with the Azure CLI 2.0](/documentation/articles/virtual-machines-linux-capture-image/). Managed disks are handled by the Azure platform and do not require any preparation or location to store them. For more information, see [Azure Managed Disks overview](/documentation/articles/storage-managed-disks-overview/). 
...
- [Azure CLI 2.0](/documentation/articles/virtual-machines-linux-capture-image/) - our next generation CLI for the resource management deployment model
...
* **Azure CLI** - Install the [Azure CLI](/documentation/articles/cli-install-nodejs/) on a local computer.
