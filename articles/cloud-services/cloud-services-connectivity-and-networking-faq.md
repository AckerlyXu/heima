...
1. Sign in to the [Azure portal](https://portal.azure.cn), go to your cloud service, and select the **Remote Desktop** tab.
...
The URL Rewrite module of IIS can be used to redirect traffic that comes to the default URL for the cloud service (for example, \*.chinacloudapp.cn) to some custom name/URL. Because the URL Rewrite module is enabled on web roles by default and its rules are configured in the application's web.config, it's always available on the VM regardless of reboots/reimages. For more information, see:
...
You can prevent incoming traffic to the default URL/name of your cloud service (for example, \*.chinacloudapp.cn). Set the host header to a custom DNS name (for example, www.MyCloudService.com) under site binding configuration in the cloud service definition (*.csdef) file, as indicated: 
 
