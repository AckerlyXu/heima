<properties 
pageTitle="Common startup tasks for Cloud Services | Azure" 
...
Variables can also use a [valid Azure XPath value](/documentation/articles/cloud-services-role-config-xpath/) to reference something about the deployment. Instead of using the `value` attribute, define a [RoleInstanceValue] child element.
