...
Deployment of a cloud service may fail if the resources that are required to be allocated exceed the default or maximum quota allowed for your service at the region/datacenter level. 
...
As for the cost, it is very small and you can reuse the same storage account across all deployments. You can use the [storage cost calculator](https://www.azure.cn/pricing/calculator/) to determine the cost to upload the service package (CSPKG), download the CSPKG, then delete the CSPKG. 