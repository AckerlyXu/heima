...
* Where resource diagnostic logs and metrics are sent (Storage Account, Event Hubs).
...
* How long each log category should be retained in a storage account
    ...
    - If retention policies are set but storing logs in a Storage Account is disabled (for example, if only Event Hubs is selected), the retention policies have no effect.
...
To enable collection of resource diagnostic logs via the Azure CLI 2.0, you use the [az monitor diagnostic-settings create](/cli/monitor/diagnostic-settings#az-monitor-diagnostic-settings-create) command.
