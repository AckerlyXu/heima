...
### Would DNS resolution using the default FQDN (internal.chinacloudapp.cn) still work even when a Private Zone (for example: contoso.local) is linked to a virtual network? 
Yes. Private Zones feature does not replace the default DNS resolutions using the Azure-provided internal.chinacloudapp.cn zone, and is offered as an additional capability or enhancement. For both cases (whether relying on Azure-provided internal.chinacloudapp.cn or on your own Private Zone) it is advised to use the FQDN of the zone you want to resolve against. 
### Would the DNS suffix on virtual machines within a linked virtual network be changed to that of the Private Zone? 
No. At this time, the DNS suffix on the virtual machines in your linked virtual network will remain as the default Azure-provided suffix ("*.internal.chinacloudapp.cn"). You can however manually change this DNS suffix on your virtual machines to that of the private zone. 
...
* Reverse DNS for a Private IP that is not registered in the Private Zone (for example: Private IP for a virtual machine in a virtual network that is linked as a Resolution virtual network to a private zone) will return "internal.chinacloudapp.cn" as the DNS suffix, however this suffix will not be resolvable.   
