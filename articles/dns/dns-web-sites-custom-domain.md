---
title: Create custom DNS records for a web app | Azure
...
If your domain is already managed by Azure DNS (see [DNS domain delegation](dns-domain-delegation.md), you can use the following the example to create a CNAME record for contoso.chinacloudsites.cn.
...
Using the previously assigned variable "$rs" you can use the PowerShell command below to create the alias for the web app contoso.chinacloudsites.cn.

```powershell
Add-AzureRMDnsRecordConfig -RecordSet $rs -Cname "contoso.chinacloudsites.cn"
```
...
Once the record set "awverify" is created, assign the CNAME record set alias. In the example below, we will assign the CNAMe record set alias to awverify.contoso.chinacloudsites.cn.

```powershell
Add-AzureRMDnsRecordConfig -RecordSet $rs -Cname "awverify.contoso.chinacloudsites.cn"
```
