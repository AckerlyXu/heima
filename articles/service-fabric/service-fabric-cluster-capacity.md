...
* Gold - The infrastructure jobs can be paused for a duration of two hours per UD. Gold durability can be enabled only on full-node VM SKUs like DS15_v2, D15_v2. In general, all the VM Sizes listed at /virtual-machines/windows/sizes?toc=%2fvirtual-machines%2fwindows%2ftoc.json that are marked as 'Instance is isolated to hardware dedicated to a single customer' in the note, are full-node VMs.
