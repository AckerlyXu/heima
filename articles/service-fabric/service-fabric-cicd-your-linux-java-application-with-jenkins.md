...
4. Update the placeholder values in the ```setupentrypoint.sh``` script with the azure-storage details from step 3.
    ...
    ```
    * Replace ``[REMOTE_FILE_SHARE_LOCATION]`` with the value ``//sfjenkinsstorage1.file.core.chinacloudapi.cn/sfjenkins`` from the output of the connect in step 3 above.
...
Here, you can upload a plug-in. Select **Choose file**, and then select the **serviceFabric.hpi** file, which you downloaded under prerequisites or can download [here](https://servicefabricdownloads.blob.core.chinacloudapi.cn/jenkins/serviceFabric.hpi). When you select **Upload**, Jenkins automatically installs the plug-in. Allow a restart if requested.
