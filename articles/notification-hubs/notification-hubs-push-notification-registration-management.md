...
If you want to use [Templates](/documentation/articles/notification-hubs-templates-cross-platform-push-messages/), the device installation also hold all templates associated with that device in a JSON format (see sample above). The template names help target different templates for the same device.
...
Note that in this case, you will use only Listen rights to access your notification hubs from the device. For more information, see [Security](/documentation/articles/notification-hubs-push-notification-security/).
