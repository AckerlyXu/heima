...
portal. The Hybrid Connections service endpoint will be referred to as the "service"
...
#### Connect
The "connect" operation opens a web socket on the service, providing the name of
...
with the listener in the way described previously, and have the listener create a
...
Connections service by simply supplying a correctly-constructed URL into their web
socket client layer.
...
was initially set up, the reason for doing so will be communicated using an
...
implementations can continue to rely on a web socket client and don’t need to
...
the control channel web socket with an error, otherwise there is no reply.
