...
- Azure AD Connect can make direct IP connections to the [Azure data center IP ranges](https://www.microsoft.com/download/details.aspx?id=42064). Again, this prerequisite is applicable only when you enable the feature.
...
To roll out the feature to your users, you need to add two Azure AD URLs (https://autologon.microsoftazuread-sso.com and https://aadg.chinacloudapi.cn.nsatc.net) to users' Intranet zone settings via Group Policy in Active Directory.
...
4. Enter "https://autologon.microsoftazuread-sso.com, https://aadg.chinacloudapi.cn.nsatc.net" in the field.
...
- Sign into *https://login.partner.microsoftonline.cn/* in a new private browser session.

To test the scenario where the user doesn't have to enter the username or the password: 
- Sign into *https://login.partner.microsoftonline.cn/contoso.partner.onmschina.cn* in a new private browser session. Replace "*contoso*" with your tenant's name.
- Or sign into *https://login.partner.microsoftonline.cn/contoso.com* in a new private browser session. Replace "*contoso.com*" with a verified domain (not a federated domain) in your tenant.
