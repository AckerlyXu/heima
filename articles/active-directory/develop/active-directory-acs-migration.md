...
description: Options for moving apps & services off of Azure Access Control Service | Azure
...
```HTTP
https://mynamespace.accesscontrol.chinacloudapi.cn
```

All communication with the STS and management operations are done at this URL, with different paths for different purposes. To determine if your applications or services use ACS, monitor for any traffic to `https://{namespace}.accesscontrol.chinacloudapi.cn`.  Any traffic to this URL is traffic that is handled by ACS and needs to be discontinued.  The one exception is any traffic to `https://accounts.accesscontrol.chinacloudapi.cn` - traffic to this URL is already handled by a different service and is not affected by ACS deprecation.  You should also be sure to login to the Azure Classic Management Portal and check for any ACS namespaces in the subscriptions that you own.  ACS namespaces are listed in the **Active Directory** service, under the **Access Control Namespaces** tab.
...
- **November 2017**:  The Azure AD admin experience in the classic Azure portal [is retired](https://blogs.technet.microsoft.com/enterprisemobility/2017/09/18/marching-into-the-future-of-the-azure-ad-admin-experience-retiring-the-azure-classic-portal/). At this point, namespace management for ACS will be available at this new, dedicated URL: `http://manage.windowsazure.cn?restoreClassic=true`. This is to allow you to view your existing namespaces, enable/disable them, and delete them entirely if you wish.
- **April 2018**: ACS namespace management is no longer be available at this dedicated URL. At this point in time, you are not able to disable/enable, delete, or enumerate your ACS namespaces. The ACS management portal, however, is fully functional and located at `https://{namespace}.accesscontrol.chinacloudapi.cn`. All other components of ACS continue to operate normally as well.
- **November 2018**: All ACS components are shut down permanently. This includes the ACS management portal, the management service, STS, and token transformation rule engine. At this point, any requests sent to ACS (located at `{namespace}.accesscontrol.chinacloudapi.cn`) fail. You should have migrated all existing apps & services to other technologies well before this time period.
...
| Azure Cache | [Migrate to Azure Redis Cache](/redis-cache/cache-faq#which-azure-cache-offering-is-right-for-me) |
...
| Azure Backup | [Upgrade the Azure Backup Agent](/backup/backup-azure-file-folder-backup-faq) |
...
Unfortunately, there is not one service that provides all of these equivalent capabilities.  You should evaluate which capabilities of ACS you need, and then choose between using **Azure Active Directory**(Azure AD) or another cloud authentication service.
...
If you decide that Azure AD is the proper path forward for your applications & services, an approach is to follow [this code sample](https://github.com/Azure-Samples/active-directory-dotnet-webapp-wsfederation), which gives slightly different instructions on how to set up Ws-Federation. This code sample does not use WIF, but rather, the ASP.NET 4.5 OWIN middleware. However, the instructions for app registration are valid for apps using WIF, and do not require an Azure AD Premium license.  If you choose this approach, you heneed to understand [signing key rollover in Azure AD](/active-directory/develop/active-directory-signing-key-rollover). This approach uses the Azure AD global signing key to issue tokens. By default, WIF does not automatically refresh signing keys. When Azure AD rotates its global signing keys, your WIF implementation needs to be prepared to accept the changes.
...
If Azure AD does not meet the needs of your web application, please reach out to us and we can help you identify the best path forward.
