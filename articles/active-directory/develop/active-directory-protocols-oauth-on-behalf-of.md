...
| client_id |required | The App ID assigned to the calling service during registration with Azure AD. |
...
| resource |required | The App ID URI of the receiving service (secured resource).|
...
#### Example
The following HTTP POST requests an access token for the https://graph.chinacloudapi.cn web API. The `client_id` identifies the service that requests the access token.
...
#### Example
The following HTTP POST requests an access token for the https://graph.chinacloudapi.cn web API with a certificate. The `client_id` identifies the service that requests the access token.
...
The following example shows a success response to a request for an access token for the https://graph.chinacloudapi.cn web API.
