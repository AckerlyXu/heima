...
- [Azure CLI 2.0](/cli/overview). You can use it in your browser with Azure Cloud Shell, or you can [install](/cli/install-azure-cli) it on macOS, Linux, and Windows and run it from the command line.

## List roles

### List role definitions

To list all available role definitions, use [az role definition list](/cli/role/definition#az_role_definition_list):
...
To list the actions of a role definition, use [az role definition list](/cli/role/definition#az_role_definition_list):
...
To list the role assignments for a specific user, use [az role assignment list](/cli/role/assignment#az_role_assignment_list):
...
To list the role assignments that exist for a resource group, use [az role assignment list](/cli/role/assignment#az_role_assignment_list):
...
To assign a role to a user at the resource group scope, use [az role assignment create](/cli/role/assignment#az_role_assignment_create):
...
To assign a role to a group, use [az role assignment create](/cli/role/assignment#az_role_assignment_create):

```azurecli
az role assignment create --role <role> --assignee-object-id <assignee_object_id> --resource-group <resource_group> --scope </subscriptions/subscription_id>
```

The following example assigns the *Reader* role to the *Ann Mack Team* group with ID 22222222-2222-2222-2222-222222222222 at the subscription scope. To get the ID of the group, you can use [az ad group list](/cli/ad/group#az_ad_group_list) or [az ad group show](/cli/ad/group#az_ad_group_show).
...
To assign a role to an application, use [az role assignment create](/cli/role/assignment#az_role_assignment_create):

```azurecli
az role assignment create --role <role> --assignee-object-id <assignee_object_id> --resource-group <resource_group> --scope </subscriptions/subscription_id>
```

The following example assigns the *Virtual Machine Contributor* role to an application with object ID 44444444-4444-4444-4444-444444444444 at the *pharma-sales-projectforecast* resource group scope. To get the object ID of the application, you can use [az ad app list](/cli/ad/app#az_ad_app_list) or [az ad app show](/cli/ad/app#az_ad_app_show).
...
To remove a role assignment, use [az role assignment delete](/cli/role/assignment#az_role_assignment_delete):
...
The following example removes the *Reader* role from the *Ann Mack Team* group with ID 22222222-2222-2222-2222-222222222222 at the subscription scope. To get the ID of the group, you can use [az ad group list](/cli/ad/group#az_ad_group_list) or [az ad group show](/cli/ad/group#az_ad_group_show).
...
To list the roles that are available for assignment at a scope, use [az role definition list](/cli/role/definition#az_role_definition_list).
...
To create a custom role, use [az role definition create](/cli/role/definition#az_role_definition_create). The role definition can be a JSON description or a path to a file containing a JSON description.
...
To update a custom role, first use [az role definition list](/cli/role/definition#az_role_definition_list) to retrieve the role definition. Second, make the desired changes to the role definition. Finally, use [az role definition update](/cli/role/definition#az_role_definition_update) to save the updated role definition.
...
To delete a custom role, use [az role definition delete](/cli/role/definition#az_role_definition_delete). To specify the role to delete, use the role name or the role ID. To determine the role ID, use [az role definition list](/cli/role/definition#az_role_definition_list).
