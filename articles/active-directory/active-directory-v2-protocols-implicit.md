<properties
    pageTitle="Azure AD v2.0 Implicit Flow | Azure"
...
You can choose to validate the `id_token` in client code, but a common practice is to send the `id_token` to a backend server and perform the validation there.  Once you've validated the signature of the id_token, there are a few claims you will be required to verify.  See the [v2.0 token reference](/documentation/articles/active-directory-v2-tokens/) for more information, including [Validating Tokens](/documentation/articles/active-directory-v2-tokens/#validating-tokens/) and [Important Information About Signing Key Rollover](/documentation/articles/active-directory-v2-tokens/#validating-tokens/).  We recommend making use of a library for parsing and validating tokens - there is at least one available for most languages and platforms.
...
For more information on the claims in an id_token, see the [v2.0 endpoint token reference](/documentation/articles/active-directory-v2-tokens/).
...
| redirect_uri |recommended |The redirect_uri of your app, where authentication responses can be sent and received by your app.  It must exactly match one of the redirect_uris you registered in the portal, except it must be url encoded. |
| scope |required |A space-separated list of scopes.  For getting tokens, include all [scopes](/documentation/articles/active-directory-v2-scopes/) you require for the resource of interest. |
