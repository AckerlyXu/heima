<properties
    pageTitle="IoT real-time data streams and Azure Stream Analytics | Azure"
...
In a real-world scenario, you could have hundreds of these sensors generating events as a stream. Ideally, a gateway device would run code to push these events to [Azure Event Hubs](/home/features/event-hubs/) or [Azure IoT Hubs](/home/features/iot-hub/). Your Stream Analytics job would ingest these events from Event Hubs and run real-time analytics queries against the streams. Then, you could send the results to one of the [supported outputs](/documentation/articles/stream-analytics-define-outputs/).
