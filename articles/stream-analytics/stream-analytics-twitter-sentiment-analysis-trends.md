...
1. In the [Azure portal preview](https://portal.azure.cn), click **NEW** > and type *Event Hubs* and then select **Event Hubs** from the resulting search. Then click **Create**.
...
In the [Azure portal](https://portal.azure.cn/), click **NEW** > type **STREAM ANALYTICS** > and click the Stream Analytics tile result. Specify the following values, and then click **CREATE**.
...
Once the job is created the job will open in Azure portal preview.
...
To compare the number of mentions among topics, we'll use a [Tumbling Window](https://msdn.microsoft.com/zh-cn/library/azure/dn835055.aspx) to get the count of mentions by topic every five seconds.
