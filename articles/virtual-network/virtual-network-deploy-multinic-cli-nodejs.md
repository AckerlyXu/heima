<properties
    pageTitle="Create a VM with multiple NICs - Azure CLI 1.0 | Azure"
...
The following steps use a resource group named *IaaSStory* for the WEB servers and a resource group named *IaaSStory-BackEnd* for the DB servers. You can complete this task using the Azure CLI 1.0 (this article) or the [Azure CLI 2.0](/documentation/articles/virtual-network-deploy-static-pip-arm-cli/). The values in "" for the variables in the steps that follow create resources with settings from the scenario. Change the values, as appropriate, for your environment.
