---
title: Create, change, or delete Azure network interfaces | Azure
...
4. In the **Create network interface** blade that appears, enter, or select values for the following settings, then click **Create**:
    ...
    |Subscription|Yes|Select one of your Azure [subscriptions](../azure-glossary-cloud-terminology.md?toc=%2fvirtual-network%2ftoc.json#subscription). The VM you attach a NIC to and the VNet you connect it to must exist in the same subscription.|
    |Resource group|Yes|Select an existing [resource group](../azure-glossary-cloud-terminology.md?toc=%2fvirtual-network%2ftoc.json#resource-group) or create one. A NIC can exist in the same, or different resource group, than the VM you attach it to, or the VNet you connect it to.|
