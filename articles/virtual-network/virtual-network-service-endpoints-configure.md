---
title: Configure Azure virtual network service endpoints | Azure
...
>[!NOTE]
During preview, Vnet service endpoints feature is supported for specific regions. For the list of supported regions, refer to  the [Azure Virtual Network updates](https://www.azure.cn/what-is-new/) page.
...
- Effective Routes to validate endpoint configuration:
   ...
   To validate if service endpoint is configured correctly, "effective routes" on any NIC in the subnet shows a new "default" route with nextHopType: VirtualNetworkServiceEndpoint, per service, per region. Learn more about [troubleshooting with effective routes](/virtual-network/virtual-network-routes-troubleshoot-portal#using-effective-routes-to-troubleshoot-vm-traffic-flow)
...
1. Open [Azure portal](https://portal.azure.cn/).
Log in to Azure using your Azure account. If you don't have an Azure account, you can sign up for a trial. The account must have the necessary [permissions](#provisioning) to create a virtual network and service endpoint.
...
Use the command below to get the list of services supported for endpoints, for an Azure region.
 ```powershell
Get-AzureRmVirtualNetworkAvailableEndpointService -location chinaeast
```
...
New-AzureRmVirtualNetwork -Name "myVNet" -AddressPrefix "10.0.0.0/16" -Subnet $subnet -ResourceGroupName "myRG" -Location "ChinaNorth"
```
...
----------------- -------           ---------
Succeeded         Microsoft.Storage {chinaeast, chinanorth}
Succeeded         Microsoft.Sql     {chinaeast}
```
...
```azure-cli
az network vnet list-endpoint-services -l chinaeast
```
Output:
```
    {
...
Download the sample [Resource Manager template](https://github.com/Azure/azure-quickstart-templates/tree/master/201-vnet-2subnets-service-endpoints-storage-integration) to secure a storage account to a subnet in a VNet.
...
Learn more about [built-in roles](/active-directory/role-based-access-built-in-roles) and assigning specific permissions to [custom roles](/active-directory/role-based-access-control-custom-roles).
...
[Securing Azure Storage accounts to Virtual Networks](/storage/common/storage-network-security)
