...
An Azure resource group is a logical container into which Azure resources are deployed and managed. A resource group must be created before a virtual machine scale set. Create a resource group with the [az group create](/cli/group#az_group_create) command. In this example, a resource group named *myResourceGroup* is created in the *chinanorth* region. 
...
## Create a scale set
You create a virtual machine scale set with the [az vmss create](/cli/vmss#az_vmss_create) command. The following example creates a scale set named *myScaleSet*, and generates SSH keys if they do not exist:
...
To view a list of VM instances in a scale set, use [az vmss list-instances](/cli/vmss#az_vmss_list_instances) as follows:
...
The first column in the output shows an *InstanceId*. To view additional information about a specific VM instance, add the `--instance-id` parameter to [az vmss get-instance-view](/cli/vmss#az_vmss_get_instance_view). The following example views information about VM instance *1*:
...
To list the address and ports to connect to VM instances in a scale set, use [az vmss list-instance-connection-info](/cli/vmss#az_vmss_list_instance_connection_info):
...
When you created a scale set at the start of the tutorial, a `--image` of *UbuntuLTS* was specified for the VM instances. The Azure marketplace includes many images that can be used to create VM instances. To see a list of the most commonly used images, use the [az vm image list](/cli/vm/image#az_vm_image_list) command.
...
To see a list of VM instance sizes available in a particular region, use the [az vm list-sizes](/cli/vm#az_vm_list_sizes) command.
...
To manually increase or decrease the number of VM instances in the scale set, use [az vmss scale](/cli/vmss#az_vmss_scale). The following example sets the number of VM instances in your scale set to *3*:
...
If takes a few minutes to update the capacity of your scale set. To see the number of instances you now have in the scale set, use [az vmss show](/cli/vmss#az_vmss_show) and query on *sku.capacity*:
...
To stop one or more VM instances in a scale set, use [az vmss stop](/cli/vmss#az_vmss_stop). The `--instance-ids` parameter allows you to specify one or more VM instances to stop. If you do not specify an instance ID, all VM instances in the scale set are stopped. The following example stops instance *1*:

```azurecli
az vmss stop --resource-group myResourceGroup --name myScaleSet --instance-ids 1
```

Stopped VM instances remain allocated and continue to incur compute charges. If you instead wish the VM instances to be deallocated and only incur storage charges, use [az vmss deallocate](/cli/vmss#az_vmss_deallocate). The following example stops and deallocates instance *1*:
...
To start one or more VM instances in a scale set, use [az vmss start](/cli/vmss#az_vmss_start). The `--instance-ids` parameter allows you to specify one or more VM instances to start. If you do not specify an instance ID, all VM instances in the scale set are started. The following example starts instance *1*:
...
To restart one or more VM instances in a scale set, use [az vmss restart](/cli/vmss#az_vm_restart). The `--instance-ids` parameter allows you to specify one or more VM instances to restart. If you do not specify an instance ID, all VM instances in the scale set are restarted. The following example restarts instance *1*:
