---
title: Protect a Web API backend with Azure Active Directory and API Management | Azure
...
To secure your Web API backed using Azure Active Directory you must first have a an AAD tenant. To create an AAD tenant, sign-in to the [Azure Classic Management Portal](https://manage.windowsazure.cn) and click **New**->**App Services**->**Active Directory**->**Directory**->**Custom Create**. 
...
In this step, a Web API backend is created using Visual Studio 2013. To create Web API backend project in Visual Studio click **File**->**New**->**Project**, and choose **ASP.NET Web Application** from the **Web** templates list. Click **OK** to create the project. 
...
The next step adds the code to the Web API project. 
...
In this step the Visual Studio project is published to Azure.
...
A new application for the backend service is created in your Azure AD directory as part of the configuring and publishing process of your Web API project. In this step, permissions are granted to the Web API backend.
...
Create a file named `calcapi.json` with following contents and save it to your computer. Ensure that the `host` attribute points to your Web API backend. In this example `"host": "apimaaddemo.azurewebsites.cn"` is used.
...
```json
{
  ...
  "host": "apimaaddemo.azurewebsites.cn",
  "basePath": "/api",
  "schemes": [
    "http"
  ],
...
Navigate to the Azure AD tenant from the first step, in this example **APIMDemo** and navigate to the **Applications** tab.
