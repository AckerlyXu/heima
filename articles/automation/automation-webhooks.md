...
<properties
    pageTitle="Starting an Azure Automation runbook with a webhook | Azure"
...
For example, if you are starting the following runbook from the Azure Portal Preview and want to pass some sample WebhookData for testing, since WebhookData is an object, it should be passed as JSON in the UI.
...
## Creating a webhook
Use the following procedure to create a new webhook linked to a runbook in the Azure portal preview.

1. From the **Runbooks blade** in the Azure portal preview, click the runbook that the webhook will start to view its detail blade.
