...
5. In the Library control, type **Add-AzureRmAccount -EnvironmentName AzureChinaCloud** in the search textbox.
6. Add **Add-AzureRmAccount -EnvironmentName AzureChinaCloud** to the canvas.<br> ![Add-AzureRMAccount](./media/automation-first-runbook-graphical/search-powershell-cmdlet-addazurermaccount.png)
7. Hover over **Get Run As Connection** until a circle appears on the bottom of the shape. Click the circle and drag the arrow to **Add-AzureRmAccount -EnvironmentName AzureChinaCloud**.  The arrow that you just created is a *link*.  The runbook will start with **Get Run As Connection** and then run **Add-AzureRmAccount -EnvironmentName AzureChinaCloud**.<br> ![Create link between activities](./media/automation-first-runbook-graphical/runbook-link-auth-activities.png)
8. On the canvas, select **Add-AzureRmAccount -EnvironmentName AzureChinaCloud** and in the Configuration control pane type **Login to Azure** in the **Label** textbox.
9. Click **Parameters** and the Activity Parameter Configuration blade appears.
10. **Add-AzureRmAccount -EnvironmentName AzureChinaCloud** has multiple parameter sets, so we need to select one before we can provide parameter values.  Click **Parameter Set** and then select the **ServicePrincipalCertificate** parameter set.
