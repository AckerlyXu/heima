...
## Overview
In this tutorial, you add push notifications to the [Xamarin.iOS quick start](/documentation/articles/app-service-mobile-xamarin-ios-get-started/) project so that a push notification is sent to the device every time a record is inserted.
...
3. In the same file, override the **RegisteredForRemoteNotifications** event. In this code you are registering for a simple template notification that will be sent across all supported platforms by the server.
	...
	For more information on templates with Notification Hubs, see [Templates](/documentation/articles/notification-hubs-templates-cross-platform-push-messages/).
