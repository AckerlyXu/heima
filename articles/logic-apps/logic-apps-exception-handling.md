...
To evaluate run statuses, you can monitor logs and metrics, or publish them to any monitoring tool that you prefer. A potential option is to stream all the events through Event Hubs to [Azure Stream Analytics](/stream-analytics/). In Stream Analytics, you can write live queries based on any anomalies, averages, or failures from the diagnostic logs. You can use Stream Analytics to send information to other data sources, like to queues, topics, SQL, Azure Cosmos DB, or Power BI.
