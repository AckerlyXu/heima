...
* At the provision time, you can specify the number of worker nodes using the Azure Portal Preview, Azure PowerShell or Cross-platform command line interface.  For more information, see [Provision HDInsight clusters](/documentation/articles/hdinsight-provision-clusters/). The following screen show the worker node configuration on the Azure Portal Preview:
