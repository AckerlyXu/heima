...
3. Find the Azure assigned DNS suffix for your virtual network. This value is similar to `0owcbllr5hze3hxdja3mqlrhhe.ex.internal.chinacloudapp.cn`. For information on finding the DNS suffix, see the [Example: Custom DNS](#example-dns) section.
...
| Region | Allowed IP addresses | Allowed port | Direction |
| ---- | ---- | ---- | ----- |
| China North | 42.159.96.170</br>139.217.2.219 | 443 | Inbound |
...
Use the following PowerShell script to create a virtual network that restricts inbound traffic, but allows traffic from the IP addresses required by HDInsight in the China North region.
...
2. On the custom DNS server for the virtual network, use the following text as the contents of the `/etc/bind/named.conf.local` file:
    ...
    Replace the `0owcbllr5hze3hxdja3mqlrhhe.ex.internal.chinacloudapp.cn` value with the DNS suffix of your virtual network.
...
2. Use the following text as the contents of the `/etc/bind/named.config.local` file on the custom DNS server. Make this change on the custom DNS server in both virtual networks.
    ...
    Replace the `0owcbllr5hze3hxdja3mqlrhhe.ex.internal.chinacloudapp.cn` value with the DNS suffix of the __other__ virtual network. This entry routes requests for the DNS suffix of the remote network to the custom DNS in that network.
