...
3. After the nano editor opens, use the following XML as the contents of the file:
   ...
   * Replace all instances of **wasb://mycontainer@mystorageaccount.blob.core.chinacloudapi.cn** with the value you received earlier for the default storage.
...
* **URI**: You can access the REST API from outside the cluster at `https://CLUSTERNAME.azurehdinsight.cn/oozie`.
...
2. After you create a tunnel, open the Ambari web UI in your web browser. The URI for the Ambari site is `https://CLUSTERNAME.azurehdinsight.cn`. Replace `CLUSTERNAME` with the name of your Linux-based HDInsight cluster.
...
**Cause**: The Azure Blob storage addresses used in the **job.xml** file doesn't contain the storage container or storage account name. The Blob storage address format must be `wasb://containername@storageaccountname.blob.core.chinacloudapi.cn`.
