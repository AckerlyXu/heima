...
<properties
    pageTitle="Use Python libraries to analyze website logs in Azure Spark cluster | Azure"
...
6. You can then run the following SQL query to get all the records in the **AverageTime** table.
    ...
    For more information about the `%%sql` magic, see [Parameters supported with the %%sql magic](/documentation/articles/hdinsight-apache-spark-jupyter-notebook-kernels/#parameters-supported-with-the-sql-magic).
