...
* An **SSH** client: Used to connect to the Linux-based HDInsight clusters. For more information on using SSH with HDInsight, see the following documents:
    ...
    * [Use SSH (PuTTY) with HDInsight from a Windows client](/documentation/articles/hdinsight-hadoop-linux-use-ssh-windows/)
    * [Use SSH with HDInsight from a Linux, Unix, OS X, or Bash on Windows 10](/documentation/articles/hdinsight-hadoop-linux-use-ssh-unix/)
...
* **EventHub Spout**: The spout is provided as part of Apache Storm version 0.10.0 and higher.
    ...
    > [AZURE.NOTE]
    > The Event Hub spout used in this example requires a Storm on HDInsight cluster version 3.3 or 3.4. For information on how to use Event Hubs with an older version of Storm on HDInsight, see [Process events from Azure Event Hubs with Storm on HDInsight](/documentation/articles/hdinsight-storm-develop-java-event-hub-topology/).
    > 
    > 
...
The steps in this section use an [Azure Resource Manager template](/documentation/articles/resource-group-template-deploy/) to create a Azure Virtual Network and a Storm and HBase cluster on the virtual network. The template also creates an Azure Web App and deploys a copy of the dashboard into it.
...
For more examples of Storm topologies with HDInsight, see [Example topologies for Storm on HDInsight](/documentation/articles/hdinsight-storm-example-topology/)
