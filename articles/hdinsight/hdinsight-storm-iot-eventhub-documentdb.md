...
description: Learn how to process vehicle sensor data from Event Hubs using Apache Storm on HDInsight. Add model data from Azure DocumentDb, and store output to storage.
...
Learn how to process vehicle sensor data from Azure Event Hubs using Apache Storm on HDInsight. This example reads sensor data from Azure Event Hubs, enriches the data by referencing data stored in Azure DocumentDb. The data is stored into Azure Storage using the Hadoop File System (HDFS).
...
During processing, the VIN is used to retrieve model information from DocumentDb. This data is added to the data stream before it is stored.
