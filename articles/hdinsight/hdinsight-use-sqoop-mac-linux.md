...
1. Use SSH to connect to the Linux-based HDInsight cluster. The address to use when connecting is `CLUSTERNAME-ssh.azurehdinsight.cn` and the port is `22`.
    ...
    * **Linux, Unix or OS X clients**: See [Connect to a Linux-based HDInsight cluster from Linux, OS X or Unix](/documentation/articles/hdinsight-hadoop-linux-use-ssh-unix/)
    * **Windows clients**: See [Connect to a Linux-based HDInsight cluster from Windows](/documentation/articles/hdinsight-hadoop-linux-use-ssh-windows/)
