...
This exercise requires the Azure PowerShell module version 4.4 or later. Run `Get-Module -ListAvailable AzureRM` to find the version. If you need to install or upgrade, see [Install Azure PowerShell module](https://docs.microsoft.com/powershell/azure/install-azurerm-ps). 

For this exercise, you can type the commands into a regular PowerShell window, or you can use the [Windows PowerShell Integrated Scripting Environment (ISE)](https://docs.microsoft.com/powershell/scripting/getting-started/fundamental/windows-powershell-integrated-scripting-environment--ise-) and type the commands into an editor, then test one or more commands at a time as you go through the examples. You can highlight the rows you want to execute and click Run Selected to just run those commands.
...
To retrieve an existing storage account, you need the name of the resource group and the name of the storage account. Set the variables for those two fields, then use the [Get-AzureRmStorageAccount](https://docs.microsoft.com/powershell/module/azurerm.storage/Get-AzureRmStorageAccount) cmdlet. 
...
*   [New-AzureRmStorageAccount](https://docs.microsoft.com/powershell/module/azurerm.storage/New-AzureRmStorageAccount) -- creates the actual storage account. The example uses `testpshstorage`.
...
> [!IMPORTANT]
> The name of your storage account must be unique within Azure and must be lowercase. For naming conventions and restrictions, see [Naming and Referencing Containers, Blobs, and Metadata](https://docs.microsoft.com/rest/api/storageservices/Naming-and-Referencing-Containers--Blobs--and-Metadata).
> 
...
An Azure Storage account comes with two account keys. To retrieve the keys, use [Get-AzureRmStorageAccountKey](https://docs.microsoft.com/powershell/module/AzureRM.Storage/Get-AzureRmStorageAccountKey). This example retrieves the first key. To retrieve the other one, use `Value[1]` instead of `Value[0]`.
...
To regenerate the key, use [New-AzureRmStorageAccountKey](https://docs.microsoft.com/powershell/module/AzureRM.Storage/New-AzureRmStorageAccountKey). 
...
To delete a storage account, use [Remove-AzureRmStorageAccount](https://docs.microsoft.com/powershell/module/azurerm.storage/Remove-AzureRmStorageAccount).
...
[Azure Storage Analytics](storage-analytics.md) consists of [Storage Analytics Metrics](https://docs.microsoft.com/rest/api/storageservices/about-storage-analytics-metrics) and [Storage Analytics Logging](https://docs.microsoft.com/rest/api/storageservices/about-storage-analytics-logging). 
...
You can configure monitoring using the [Azure portal](https://portal.azure.cn), PowerShell, or programmatically using the storage client library. 
...
* To learn how to enable and retrieve Storage Logging data using PowerShell, see [How to enable Storage Logging using PowerShell](https://docs.microsoft.com/rest/api/storageservices/Enabling-Storage-Logging-and-Accessing-Log-Data#how-to-enable-storage-logging-using-powershell) and [Finding your Storage Logging log data](https://docs.microsoft.com/rest/api/storageservices/Enabling-Storage-Logging-and-Accessing-Log-Data#finding-your-storage-logging-log-data).
...
* [Azure Storage control plane PowerShell cmdlets](https://docs.microsoft.com/powershell/module/AzureRM.Storage/)
* [Azure Storage data plane PowerShell cmdlets](https://docs.microsoft.com/powershell/module/azure.storage/)
