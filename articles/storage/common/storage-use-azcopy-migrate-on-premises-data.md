...
To complete this tutorial, download the latest version of AzCopy on [Linux](storage-use-azcopy-linux.md#download-and-install-azcopy) or [Windows](http://aka.ms/downloadazcopy). 

[!INCLUDE [storage-quickstart-tutorial-create-account-portal](../../../includes/storage-quickstart-tutorial-create-account-portal.md)]

>[!NOTE]
>If you want to be able to download blobs from a secondary region to your local storage and vice versa, set **Replication** to **Read-access-geo-redundant storage**. Selecting this option creates a [geo-redundant storage](storage-redundancy.md#geo-redundant-storage) account. 
>
>
...
Container names must start with a letter or number. They can contain only letters, numbers, and the hyphen character (-). For more rules about naming blobs and containers, see [Naming and referencing containers, blobs, and metadata](https://docs.microsoft.com/rest/api/storageservices/naming-and-referencing-containers--blobs--and-metadata).

## Upload all files in a folder to Blob storage

You can use AzCopy to upload all files in a folder to Blob storage on [Windows](storage-use-azcopy.md#upload-blobs-to-blob-storage) or [Linux](storage-use-azcopy-linux.md#upload-all-files). To upload all blobs in a folder, enter the following AzCopy command:
...
You can use AzCopy to [upload files](storage-use-azcopy.md#other-azcopy-features) based on their last-modified time. To try this, modify or create new files in your source directory for test purposes. To upload only updated or new files, add the `--exclude-older` (Linux) or `/XO` (Windows) parameter to the AzCopy command.
...
To learn more about ways to move on-premises data to Azure Storage and vice versa, see [Move data to and from Azure Storage](storage-moving-data.md).  
