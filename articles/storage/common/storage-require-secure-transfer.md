...
You can turn on the "Secure transfer required" setting when you create a storage account in the [Azure portal](https://portal.azure.cn). You can also enable it for existing storage accounts.
...
This sample requires the Azure PowerShell module version 4.1 or later. Run ` Get-Module -ListAvailable AzureRM` to find the version. If you need to install or upgrade, see [Install Azure PowerShell module](https://docs.microsoft.com/powershell/azure/install-azurerm-ps).
...
<!--Update_Desription: update PowerShell and CLI sample for enable "Secure transfer required" setting-->