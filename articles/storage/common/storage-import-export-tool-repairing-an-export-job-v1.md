---
title: Repairing an Azure Import/Export export job - v1 | Azure
...
After an export job has completed, you can run the Azure Import/Export Tool on-premises to:  
...
|**/ManifestFile:<DriveManifestFile\>**|Optional. The path to the export drive's manifest file. This file is generated by the Azure Import/Export service and stored on the export drive, and optionally in a blob in the storage account associated with the job.<br /><br /> The content of the files on the export drive will be verified with the MD5 hashes contained in this file. Any files that are determined to be corrupted will be downloaded and rewritten to the target directories.|  
