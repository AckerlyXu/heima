...
For more general information on Azure Storage Accounts, see [About Azure storage accounts](/storage/storage-create-storage-account).
...
- SAS tokens, constructed using Key Vault storage account keys, provide even more controlled access to an Azure storage account. For more information, see [Using shared access signatures](/storage/storage-dotnet-shared-access-signature-part-1).
