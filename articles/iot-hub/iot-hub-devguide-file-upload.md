...
The device is responsible for uploading the file to storage using the Azure Storage SDKs. When the upload is complete, the device sends a POST request to `{iot hub}.azure-devices.cn/devices/{deviceId}/files/notifications` with the following JSON body:
