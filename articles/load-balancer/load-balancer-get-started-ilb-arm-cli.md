---
title: Create an internal Basic Load Balancer - Azure CLI 2.0 | Azure
...
The following example creates a resource group named *myResourceGroupILB* in the *chinaeast* location:
...
Create two network interfaces with [az network nic create](https://docs.azure.cn/zh-cn/cli/network/nic?view=azure-cli-latest#az_network_nic_create) and associate them with the private IP address and the network security group. 
...
To get the private IP address of the load balancer, use [az network lb show](https://docs.azure.cn/zh-cn/cli/network/public-ip?view=azure-cli-latest##az-network-lb-show). Copy the private IP address, and then paste it into the address bar of a web browser of your virtual machine - *myVMTest*.
