...
* Install [Visual Studio 2017](https://www.visualstudio.com/visual-studio-homepage-vs.aspx) with the following workloads:
...
The sample project contains a basic [ASP.NET MVC](https://www.asp.net/mvc) CRUD (create-read-update-delete) app using [Entity Framework Code First](https://docs.microsoft.com/aspnet/mvc/overview/getting-started/getting-started-with-ef-using-mvc/creating-an-entity-framework-data-model-for-an-asp-net-mvc-application).
...
You can keep the generated web app name, or change it to another unique name (valid characters are `a-z`, `0-9`, and `-`). The web app name is used as part of the default URL for your app (`<app_name>.chinacloudsites.cn`, where `<app_name>` is your web app name). The web app name needs to be unique across all apps in Azure. 
...
A unique server name is generated. This name is used as part of the default URL for your logical server, `<server_name>.database.chinacloudapi.cn`. It must be unique across all logical server instances in Azure. You can change the server name, but for this tutorial, keep the generated value.

Add an administrator username and password, and then select **OK**. For password complexity requirements, see [Password Policy](https://docs.microsoft.com/sql/relational-databases/security/password-policy).
