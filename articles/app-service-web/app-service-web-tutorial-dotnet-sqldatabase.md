...
You can keep the generated web app name, or change it to another unique name. The web app name is used as part of the default URL for your app (`<app_name>.chinacloudsites.cn`). The web app name needs to be unique across all apps in Azure. 
...
A unique server name is generated. This name is used as part of the default URL for your database server, `<server_name>.database.chinacloudapi.cn`. It must be unique across all SQL Server instances in Azure. You can change the server name, but for this tutorial, keep the generated value.
