...
With the release of Azure PowerShell version 1.1.0, a new option has been added to `New-AzureRMWebApp` that lets you clone an existing Web App to a newly created app in a different region or in the same region. This option enables customers to deploy a number of apps across different regions quickly and easily.
...
Scenario: An existing web app in China East region, and you want to clone the contents to a new web app in China North region. It can be accomplished by using the Azure Resource Manager version of the PowerShell cmdlet to create a new web app with the -SourceWebApp option.
...
Using the New-AzureRmWebApp command, you can create the new web app in the China North region, and tie it to an existing premium tier App Service Plan. Moreover,you can use the same resource group as the source web app, or define a new resource group, as shown in the following command:
