---
title: Authenticate and authorize users end-to-end in Azure App Service 
...
az group create --name myAuthResourceGroup --location "China North"
...
```
http://<back_end_app_name>.chinacloudapi.cn
http://<front_end_app_name>.chinacloudapi.cn
```
...
private static readonly string _remoteUrl = "https://<back_end_app_name>.chinacloudapi.cn";
```
...
Navigate to `http://<front_end_app_name>.chinacloudapi.cn` and add a few items, such as `from front end 1` and `from front end 2`.

Navigate to `http://<back_end_app_name>.chinacloudapi.cn` to see the items added from the front-end app. Also, add a few items, such as `from back end 1` and `from back end 2`, then refresh the front-end app to see if it reflects the changes.
...
If you like, navigate to `http://<front_end_app_name>.chinacloudapi.cn.net`. It should now direct you to a sign-in page. After you sign in, you still can't access the data from the back-end app, because you still need to do three things:
...
Sign in to `http://<front_end_app_name>.chinacloudapi.cn.net` again. At the user data usage agreement page, click **Accept**.
...
In Line 51, set the `apiEndpoint` variable to the URL of your back-end app (`http://<back_end_app_name>.chinacloudapi.cn.net`). Replace _\<back\_end\_app\_name>_ with your app name in App Service.
...
Navigate to `http://<front_end_app_name>.chinacloudapi.cn.net` again. You should now be able to create, read, update, and delete data from the back-end app, directly in the Angular.js app.
