...
- Before you can create your first function, you need to have an active Azure account. If you don't already have an Azure account, [1rmb trial accounts are available](https://www.azure.cn/zh-cn/pricing/1rmb-trial-full/?form-type=identityauth).
...
1. Go to the [Azure portal](http://portal.azure.cn) and sign-in with your Azure account.
2. Create a function app as described [here](../azure-functions/functions-create-function-app-portal.md).

>[!NOTE]
> A storage account that you specify in the **ConnectionString** environment variable (see the next step) should be in the same region as your app.
...
**ConnectionString**: storage connection of the account associated with the Media Services account. This value is used in the **function.json** file and **run.csx** file (described below).
