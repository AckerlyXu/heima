<properties 
	pageTitle="Use Azure Queue storage to monitor Media Services job notifications with .NET | Azure" 
...
When you run jobs, you often require a way to track job progress. You can check the progress by using Azure Queue storage to monitor Media Services job notifications (as described in this topic) or defining a StateChanged event handler (as described in [this](/documentation/articles/media-services-check-job-progress/) topic.  

## Use Azure Queue storage to monitor Media Services job notifications

Azure Media Services has the ability to deliver notification messages to the [Azure Queue storage](/documentation/articles/storage-dotnet-how-to-use-queues/#what-is) when processing media jobs. This topic shows how to get these notification messages from Queue storage.
