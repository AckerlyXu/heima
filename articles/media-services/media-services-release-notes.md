<properties
    pageTitle="Media Services release notes | Azure"
...
Some clients can come aross a repeat tag issue in the Smooth Streaming manifest.|For more information, see [this](/documentation/articles/media-services-deliver-content-overview/#known-issues) section.
...
Azure Media Services now enables you to access telemetry/metrics data for its services. The current version of AMS lets you collect telemetry data for live Channel, StreamingEndpoint, and live Archive entities. For more information, see [this](/documentation/articles/media-services-telemetry-overview/) topic.

##<a id="july_changes16"></a>July 2016 Release

###Updates to manifest file (*.ISM) generated by encoding tasks

When an encoding task is submitted to Media Encoder Standard or Azure Media Encoder, the encoding task generates a [streaming manifest file](/documentation/articles/media-services-deliver-content-overview/) (*.ism) file in the output Asset. With the latest service release, the syntax of this streaming manifest file has been updated.
...
Azure Media Servces introduced Azure Media Analytics for powerful video intelligence. For detailed information, see [Azure Media Services Analytics Overview](/documentation/articles/media-services-analytics-overview/).

### Apple FairPlay (Preview)

Azure Media Services now enables you to dynamically encrypt your HTTP Live Streaming (HLS) content with  Apple FairPlay. You can also use AMS license delivery service to deliver FairPlay licenses to clients. For more detailed information, see [Use Azure Media Services to Stream your HLS content Protected with Apple FairPlay ](/documentation/articles/media-services-protect-hls-with-fairplay/).
...
- Starting with Media Services REST 2.10, channels that are configured to ingest an RTMP protocol, are created with primary and secondary ingest URLs. For more information, see [Channel ingest configurations](/documentation/articles/media-services-live-streaming-with-onprem-encoders/#channel_input)
...
- Note that you can only stream over SSL if the streaming endpoint from which you deliver your content was created after September 10th, 2014. If your streaming URLs are based on the streaming endpoints created after September 10th, the URL contains “streaming.mediaservices.chinacloudapi.cn” (the new format). Streaming URLs that contain “origin.mediaservices.chinacloudapi.cn” (the old format) do not support SSL. If your URL is in the old format and you want to be able to stream over SSL, [create a new streaming endpoint](/documentation/articles/media-services-portal-manage-streaming-endpoints/). Use URLs created based on the new streaming endpoint to stream your content over SSL.
...
[Media Services Pricing Details]: /pricing/details/media-services/
