...
## Dynamic encryption
 You can use Media Services to deliver your content encrypted dynamically with AES clear key or DRM encryption by using PlayReady or FairPlay. Currently, you can encrypt the HTTP Live Streaming (HLS), MPEG DASH, and Smooth Streaming formats. Encryption on progressive downloads is not supported. Each encryption method supports the following streaming protocols:
...
* The following encryption types can be specified:
  * **cenc**: For PlayReady (common encryption)
