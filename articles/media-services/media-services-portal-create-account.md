<properties
    pageTitle=" Create an Azure Media Services account with the Azure portal | Azure"
...
3. In **CREATE MEDIA SERVICES ACCOUNT** enter required values.
   ...
   3. In **Resource Group**, select the new or existing resource.  A resource group is a collection of resources that share lifecycle, permissions, and policies. Learn more [here](/documentation/articles/resource-group-overview/#resource-groups).
   ...
   5. In **Storage Account**, select a storage account to provide blob storage of the media content from your Media Services account. You can select an existing storage account in the same geographic region as your Media Services account, or you can create a storage account. A new storage account is created in the same region. The rules for storage account names are the same as for Media Services accounts.
       ...
       Learn more about storage [here](/documentation/articles/storage-introduction/).
...
## Next steps
You can now upload files into your AMS account. For more information, see [Upload files](/documentation/articles/media-services-portal-upload-files/).
