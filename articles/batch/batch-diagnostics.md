<properties
    pageTitle="Enable diagnostic logging for Batch events - Azure | Azure"
...
> [AZURE.NOTE]
> This article discusses logging events for Batch account resources themselves, not job and task output data. For details on storing the output data of your jobs and tasks, see [Persist Azure Batch job and task output](/documentation/articles/batch-task-output/).
> 
> 

## Prerequisites
- [Azure Batch account](/documentation/articles/batch-account-create-portal/)
- [Azure Storage account](/documentation/articles/storage-create-storage-account/#create-a-storage-account/)
  
  To persist Batch diagnostic logs, you must create an Azure Storage account where Azure will store the logs. You specify this Storage account when you [enable diagnostic logging](#enable-diagnostic-logging) for your Batch account. The Storage account you specify when you enable log collection is not the same as a linked storage account referred to in the [application packages](/documentation/articles/batch-application-packages/) and [task output persistence](/documentation/articles/batch-task-output/) articles.
...
## Next steps
In addition to storing diagnostic log events in an Azure Storage account, you can also stream Batch Service Log events to an [Azure Event Hub](/documentation/articles/event-hubs-what-is-event-hubs/).
