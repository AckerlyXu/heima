<properties
    pageTitle="Use job preparation and release tasks - Azure Batch | Azure"
...
> [AZURE.TIP]
> Another way to persist logs and other job and task output data is to use the [Azure Batch File Conventions](/documentation/articles/batch-task-output/) library.
> 
> 
...
The job preparation task is executed only on nodes that are scheduled to run a task. This prevents the unnecessary execution of a preparation task in case a node is not assigned a task. This can occur when the number of tasks for a job is less than the number of nodes in a pool. It also applies when [concurrent task execution](/documentation/articles/batch-parallel-node-tasks/) is enabled, which leaves some nodes idle if the task count is lower than the total possible concurrent tasks. By not running the job preparation task on idle nodes, you can spend less money on data transfer charges.
