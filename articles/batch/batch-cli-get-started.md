<properties
    pageTitle="Get started with Azure CLI for Batch | Azure"
...
- [Connect the Azure CLI to your Azure subscription](/documentation/articles/xplat-cli-connect/)
...
You can (optionally) link a **General purpose** Storage account to your Batch account when you create it. The [application packages](/documentation/articles/batch-application-packages/) feature of Batch uses blob storage in a linked General purpose Storage account, as does the [Batch File Conventions .NET](/documentation/articles/batch-task-output/) library. These optional features assist you in deploying the applications your Batch tasks run, and persisting the data they produce.
...
As mentioned in the [Batch feature overview](/documentation/articles/batch-api-basics/#pool/), you have two options when you select an operating system for the nodes in your pool: **Virtual Machine Configuration** and **Cloud Services Configuration**. Use the `--image-*` options to create Virtual Machine Configuration pools, and `--os-family` to create Cloud Services Configuration pools. You can't specify both `--os-family` and `--image-*` options.

You can specify pool [application packages](/documentation/articles/batch-application-packages/) and the command line for a [start task](/documentation/articles/batch-api-basics/#start-task/). To specify resource files for the start task, however, you must instead use a [JSON file](#json-files).
...
> [AZURE.TIP]
> Check the [list of virtual machine images](/documentation/articles/batch-linux-nodes/#list-of-virtual-machine-images/) for values appropriate for the `--image-*` options.
> 
> 
...
For details on the three clauses and performing list queries with them, see [Query the Azure Batch service efficiently](/documentation/articles/batch-efficient-list-queries/).
...
You can find more information on application packages in [Application deployment with Azure Batch application packages](/documentation/articles/batch-application-packages/).
...
## Next steps
- See [Application deployment with Azure Batch application packages](/documentation/articles/batch-application-packages/) to find out how to use this feature to manage and deploy the applications you execute on Batch compute nodes.
