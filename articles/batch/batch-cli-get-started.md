...
This article provides an overview of using [Azure CLI version 2.0](/cli/overview) with Batch. See [Get started with Azure CLI 2.0](/cli/get-started-with-azure-cli) for an overview of using the CLI with Azure.
...
To install the Azure CLI, follow the steps outlined in [Install the Azure CLI](/cli/install-azure-cli).
...
Additionally, refer to the Azure CLI reference documentation for details about [Azure CLI commands for Batch](/cli/batch). 
...
There are a few different ways to log into Azure, described in detail in [Log in with Azure CLI 2.0](/cli/authenticate-azure-cli):
...
For the purposes of this article, we show how to log into Azure interactively. Type [az login](/cli/reference-index#az_login) on the command line:
...
To use the Azure CLI to manage Batch resources, such as pools, jobs, and tasks, you need to log into your Batch account and authenticate. To log in to the Batch service, use the [az batch account login](/cli/batch/account#az_batch_account_login) command. 
...
- For more information about the Azure CLI, see the [Azure CLI documentation](/cli/overview).
