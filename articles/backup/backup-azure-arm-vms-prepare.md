<properties
    pageTitle="Azure Backup: Prepare to back up virtual machines | Azure"
...
> [AZURE.SELECTOR]
- [Resource Manager model](/documentation/articles/backup-azure-arm-vms-prepare/)
- [Classic model](/documentation/articles/backup-azure-vms-prepare/)
...
> [AZURE.NOTE]
> Azure has two deployment models for creating and working with resources: [Resource Manager and Classic](/documentation/articles/resource-manager-deployment-model/). See [Prepare your environment to back up Azure virtual machines](/documentation/articles/backup-azure-vms-prepare/) for details on working with Classic deployment model VMs.
>
>
...
If you know these conditions already exist in your environment then proceed to the [Back up your VMs article](/documentation/articles/backup-azure-vms/). If you need to set up, or check, any of these prerequisites, this article leads you through the steps to prepare that prerequisite.

##Supported operating system for backup
 - **Linux**: Azure Backup supports [a list of distributions that are endorsed by Azure](/documentation/articles/virtual-machines-linux-endorsed-distros?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json/) except Core OS Linux. _Other Bring-Your-Own-Linux distributions also might work as long as the VM agent is available on the virtual machine and support for Python exists. However, we do not endorse those distributions for backup._
...
- Restoring a domain controller (DC) VM that is part of a multi-DC configuration is supported only through PowerShell. Read more about [restoring a multi-DC domain controller](/documentation/articles/backup-azure-restore-vms/#restoring-domain-controller-vms/).
- Restoring virtual machines that have the following special network configurations is supported only through PowerShell. VMs created using the restore workflow in the UI will not have these network configurations after the restore operation is complete. To learn more, see [Restoring VMs with special network configurations](/documentation/articles/backup-azure-restore-vms/#restoring-vms-with-special-network-configurations/).
...
| --- | --- | --- |
| Installing the VM Agent |Download and install the [agent MSI](http://go.microsoft.com/fwlink/?LinkID=394789&clcid=0x409). You will need Administrator privileges to complete the installation. |<li> Install the latest [Linux agent](/documentation/articles/virtual-machines-linux-agent-user-guide/). You will need Administrator privileges to complete the installation. We recommend installing agent from your distribution repository. We **do not recommend** installing Linux VM agent directly from github.  |
| Updating the VM Agent |Updating the VM Agent is as simple as reinstalling the [VM Agent binaries](http://go.microsoft.com/fwlink/?LinkID=394789&clcid=0x409). <br>Ensure that no backup operation is running while the VM agent is being updated. |Follow the instructions on [updating the Linux VM Agent](/documentation/articles/virtual-machines-linux-update-agent?toc=%2fazure%2fvirtual-machines%2flinux%2ftoc.json/). We recommend updating agent from your distribution repository. We **do not recommend** updating Linux VM agent directly from github.<br>Ensure that no backup operation is running while the VM Agent is being updated. |
...
- [Back up virtual machines](/documentation/articles/backup-azure-vms/)
- [Plan your VM backup infrastructure](/documentation/articles/backup-azure-vms-introduction/)
- [Manage virtual machine backups](/documentation/articles/backup-azure-manage-vms-classic/)
