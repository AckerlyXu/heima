...
Currently, Azure Backup doesn’t support disks that are [larger than 1,023 GB](/backup/backup-azure-arm-vms-prepare#limitations-when-backing-up-and-restoring-a-vm). If you have disks that are larger than 1 TB:  
1. [Attach new disks](/virtual-machines/windows/attach-managed-disk-portal) that are smaller than 1 TB.  
...
1. Follow the instructions for [updating the Linux VM agent](../virtual-machines/linux/update-agent.md).
    ...
    > [!NOTE]
    > We *strongly recommend* that you update the agent only through a distribution repository. We do not recommend downloading the agent code directly from GitHub and updating it. If the latest agent for your distribution is not available, contact distribution support for instructions on how to install it. To check for the most recent agent, go to the [Azure Linux agent](https://github.com/Azure/WALinuxAgent/releases) page in the GitHub repository.
...
1. In the [Azure portal](https://portal.azure.cn/), go to the VM that is experiencing backup failure.
2. Select **Settings**.
3. Select **Extensions**.
...
4. Get the restore point collection that corresponds to the VM: <br>
    ...
    Example: `.\armclient.exe get https://management.chinacloudapi.cn/subscriptions/f2edfd5d-5496-4683-b94f-b3588c579006/resourceGroups/winvaultrg/providers/Microsoft.Compute/restorepointcollections/AzureBackup_winmanagedvm?api-version=2017-03-30`
