---
title: View relative latencies to Azure regions from specific locations | Azure
...
If you already have a network watcher in at least one Azure [region](https://www.azure.cn/regions), you can skip the tasks in this section. Create a resource group for the network watcher. In this example, the resource group is created in the China East region, but you can create the resource group in any Azure region.

```powershell
New-AzureRmResourceGroup -Name NetworkWatcherRG -Location chinaeast
```

Create a network watcher. You must have a network watcher created in at least one Azure region. In this example, a network watcher is created in the China East Azure region.

```powershell
New-AzureRmNetworkWatcher -Name NetworkWatcher_chinaeast -ResourceGroupName NetworkWatcherRG -Location chinaeast
```

## Compare relative network latencies to a single Azure region from a specific location

Evaluate service providers, or troubleshoot a user reporting an issue such as "the site was slow," from a specific location to the azure region where a service is deployed. For example, the following command returns the average relative Internet service provider latencies between the city of Beijing in China and the China North Azure region between December 13-15, 2017:
...
>[!NOTE]
Unlike when you specify a single location, if you don't specify a location, or specify multiple locations, such as "China North2", "China North", you must specify an Internet service provider when running the command. 
...
```powershell
Get-AzureRmNetworkWatcherReachabilityProvidersList -NetworkWatcherName NetworkWatcher_chinaeast -ResourceGroupName NetworkWatcherRG
```

Data is only available for the countries, states, and cities returned by the previous command. The previous command requires you to specify an existing network watcher. The example specified the *NetworkWatcher_chinaeast* network watcher in a resource group named *NetworkWatcherRG*, but you can specify any existing network watcher. If you don't have an existing network watcher, create one by completing the tasks in [Create a network watcher](#create-a-network-watcher). 
