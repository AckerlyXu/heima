<properties
   pageTitle="Configure Expressroute and Site-to-Site VPN connections that can coexist for the Resource Manager deployment model | Azure"
...
- **Basic SKU gateway is not supported.** You must use a non-Basic SKU gateway for both the [ExpressRoute gateway](/documentation/articles/expressroute-about-virtual-network-gateways/) and the [VPN gateway](/documentation/articles/vpn-gateway-about-vpngateways/).
- **Only route-based VPN gateway is supported.** You must use a route-based [VPN Gateway](/documentation/articles/vpn-gateway-about-vpngateways/).
...
6. <a name="vpngw"></a>Next, create your Site-to-Site VPN gateway. For more information about the VPN gateway configuration, see [Configure a VNet with a Site-to-Site connection](/documentation/articles/vpn-gateway-create-site-to-site-rm-powershell/). The GatewaySKU must be *Standard*, *HighPerformance*, or *UltraPerformance*. The VpnType must *RouteBased*.
	...
	You can find the BGP peering IP and the AS number that Azure uses for the VPN gateway in $azureVpn.BgpSettings.BgpPeeringAddress and $azureVpn.BgpSettings.Asn. For more information, see [Configure BGP](/documentation/articles/vpn-gateway-bgp-resource-manager-ps/) for Azure VPN gateway.
