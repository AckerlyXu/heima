<properties
   pageTitle="Azure ExpressRoute for Cloud Solution Providers | Azure"
...
-	**Role Based Access Control (RBAC)** – RBAC is based on Azure Active Directory.  For more information on Azure RBAC see [here](/documentation/articles/role-based-access-control-configure/).
-	**Networking** – Covers the various topics of networking in Azure.
-	**Azure Active Directory (AAD)** – AAD provides the identity management for Microsoft Azure and 3rd party SaaS applications. For more information about Azure AD see [here](/documentation/services/active-directory/).  
...
### Connect-through model
In a connect-through configuration the you will be responsible for all of the networking underpinnings to connect your customers datacenter resources to the subscriptions hosted in Azure. Each of your customer's that want to use Azure capabilities will need their own ExpressRoute connection, which will be managed by the You. The you will use the same methods the customer would use to procure the ExpressRoute circuit. The you will follow the same steps outlined in the article [ExpressRoute workflows](/documentation/articles/expressroute-workflows/) for circuit provisioning and circuit states. The you will then configure the Border Gateway Protocol (BGP) routes to control the traffic flowing between the on-premises network and Azure vNet.
...
ExpressRoute offers three routing domains: public, private peering. Each of the routing domains are configured with identical routers in active-active configuration for high availability. For more details on ExpressRoute routing domains look [here](/documentation/articles/expressroute-circuit-peerings/).

You can define custom routes filters to allow only the route(s) you want to allow or need. For more information or to see how to make these changes see article: [Create and modify routing for an ExpressRoute circuit using PowerShell](/documentation/articles/expressroute-howto-routing-classic/) for more details about routing filters.

>[AZURE.NOTE] For Microsoft and Public Peering connectivity must be though a public IP address owned by the customer or CSP and must adhere to all defined rules. For more information, see the [ExpressRoute Prerequisites](/documentation/articles/expressroute-prerequisites/) page.  
...
User-defined routes allow the control of traffic outbound from the assigned subnet to other subnets in the virtual network or over one of the other predefined gateways (ExpressRoute; internet or VPN). The default system routing table can be replaced with a user-defined routing table that replaces the default routing table with custom routes. With user-defined routing, customers can create specific routes to appliances such as firewalls or intrusion detection appliances, or block access to specific subnets from the subnet hosting the user-defined route. For an overview of User Defined Routes look [here](/documentation/articles/virtual-networks-udr-overview/). 
...
ExpressRoute connection to the on premises datacenter. For more information about Forced tunneling look [here](/documentation/articles/expressroute-routing/#advertising-default-routes).  
