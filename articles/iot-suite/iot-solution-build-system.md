...
MyDriving is an Internet of Things (IoT) solution that gathers data from your car, processes it by using machine learning, and presents it on your mobile phone. The back end consists of a variety of services provided by Azure. The clients can be Android, iOS, or Windows 10 phones.
...
There's a [longer description of this setup](https://msdn.microsoft.com/zh-cn/library/mt613162.aspx).
...
-   [Xamarin Studio for iOS](https://developer.xamarin.com/guides/ios/getting_started/installation/mac/). You can also set up Visual Studio and Xamarin on a Mac that's running a Windows virtual machine. See [Setup, install, and verifications for Mac users](https://msdn.microsoft.com/zh-cn/library/mt488770.aspx) on MSDN.

-   [Azure development tools](/downloads/) (optional).
...
-   Use PowerShell:
 ...
 -   *location* is the Azure location, such as `China North` or `China North`. Use `Get-AzureLocation` to find a list of available locations.
...
[Application Insights](/documentation/articles/app-insights-overview/) collects telemetry about the performance and usage of your web services. The Application Insights SDK sends telemetry from the service to the Application Insights resource in Azure.
...
You can add custom telemetry (or instrument an application that's running somewhere outside Azure) by [adding the Application Insights SDK](/documentation/articles/app-insights-asp-net/) at development time. This is useful to log metrics that depend on the application, such as users' average trip length or total mileage. In Visual Studio, right-click the project, and then select **Add Application Insights**.
...
Just to be sure that your web service is always up and running, you can set up [availability tests](/documentation/articles/app-insights-monitor-web-app-availability/). These tests ping your site from various locations around the world every 15 minutes. Again, you'll get an email if there seems to be a problem.
