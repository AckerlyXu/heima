<properties
    pageTitle="Power BI tutorial for DocumentDB connector | Azure"
...
- Access to our demo account or data in your Azure DocumentDB account.
  ...
  - Or, to create your own account, see [Create a DocumentDB database account using the Azure portal](/documentation/articles/documentdb-create-account/). Then, to get sample volcano data that's similar to what's used in this tutorial (but does not contain the GeoJSON blocks), see the [NOAA site](https://www.ngdc.noaa.gov/nndc/struts/form?t=102557&s=5&d=5) and then import the data using the [DocumentDB data migration tool](/documentation/articles/documentdb-import-data/).
...
7. If you are connecting to this endpoint for the first time, you will be prompted for the account key.  You can retrieve the key from the **Primary Key** box in the **[Read-only Keys](/documentation/articles/documentdb-manage-account/#keys/)** blade of the Azure portal, or you can use the demo account, in which case the key is `MSr6kt7Gn0YRQbjd6RbTnTt7VHc5ohaAFu7osF0HdyQmfR+YhwCH2D2jcczVIR1LNK3nMPNBD31losN7lQ/fkw==`. Enter the account key and click **Connect**.
   
    We recommend that you use the read-only key when building reports.  This will prevent unnecessary exposure of the master key to potential security risks. The read-only key is available from the [Keys](/documentation/articles/documentdb-manage-account/#keys/) blade of the Azure portal or you can use the demo account information provided above.
...
- To learn more about DocumentDB, see the [DocumentDB documentation landing page](/documentation/services/documentdb/).
