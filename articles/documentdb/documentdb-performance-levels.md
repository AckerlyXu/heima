...
The following table compares the throughput and storage options available in single partition collections, partitioned collections, and S1, S2, S3 performance levels. 
...
1. In the [**Azure portal**](https://portal.azure.cn), click **DocumentDB**, then select the DocumentDB account to modify. 
 
    If **DocumentDB** is not on the Jumpbar, click >, scroll to **Databases**, select **DocumentDB**, and then select the DocumentDB account.  
...
## Next steps
To learn more about pricing and managing data with DocumentDB, explore these resources:

1. [Partitioning data in DocumentDB](documentdb-partition-data.md). Understand the difference between single partition container and partitioned containers, as well as tips on implementing a partitioning strategy to scale seamlessly.
