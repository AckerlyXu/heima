<properties
    pageTitle="Azure DocumentDB portal tool: Document Explorer | Azure"
...
This article provides an overview of the [Azure DocumentDB](/home/features/documentdb/) Document Explorer, an Azure portal tool that enables you to view, edit, create, upload, and filter JSON documents with DocumentDB. 
...
3. The filter settings appear below the command bar. In the filter settings, provide a WHERE clause and/or an ORDER BY clause, and then click **Filter**.
	...
	Document Explorer automatically refreshes the results with documents matching the filter query. Read more about the DocumentDB SQL grammar in the [SQL query and SQL syntax](/documentation/articles/documentdb-sql-query/) article or print a copy of the [SQL query cheat sheet](/documentation/articles/documentdb-sql-query-cheat-sheet/).
...
The Document Explorer in the Azure portal is just one way to work with documents in DocumentDB. You can also work with documents using the [REST API](https://msdn.microsoft.com/zh-cn/library/azure/mt489082.aspx) or the [client SDKs](/documentation/articles/documentdb-sdk-dotnet/). For example code, see the [.NET SDK document examples](/documentation/articles/documentdb-dotnet-samples/#document-examples/) and the [Node.js SDK document examples](/documentation/articles/documentdb-nodejs-samples/#document-examples/).

If you need to import or migrate files from another source (JSON files, MongoDB, SQL Server, CSV files, Azure Table storage, Amazon DynamoDB, or HBase), you can use the DocumentDB [data migration tool](/documentation/articles/documentdb-import-data/) to quickly import your data to DocumentDB.
...
**Explanation**: The portal is an application like any other, making calls to your DocumentDB database and collection. If your requests are currently being throttled due to calls being made from a separate application, the portal may also be throttled, causing resources not to appear in the portal. To resolve the issue, address the cause of the high throughput usage, and then refresh the portal blade. Information on how to measure and lower throughput usage can be found in the [Throughput](/documentation/articles/documentdb-performance-tips/#throughput/) section of the [Performance tips](/documentation/articles/documentdb-performance-tips/) article.

## Next steps

To learn more about the DocumentDB SQL grammar supported in Document Explorer, see the [SQL query and SQL syntax](/documentation/articles/documentdb-sql-query/) article or print out the [SQL query cheat sheet](/documentation/articles/documentdb-sql-query-cheat-sheet/).
