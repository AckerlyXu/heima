<properties
    pageTitle="Create a DocumentDB database and collection | Azure"
...
To use Azure DocumentDB, you must have a [DocumentDB account](/documentation/articles/documentdb-create-account/), a database, a collection, and documents. This topic describes how to create a DocumentDB collection in the Azure portal.

Not sure what a collection is? See [What is a DocumentDB collection?](#what-is-a-documentdb-collection)

1. In the [Azure portal](https://portal.azure.cn/), in the Jumpbar, click **DocumentDB (NoSQL)**, and then in the **DocumentDB (NoSQL)** blade, select the account in which to add a collection. If you don't have any accounts listed, you'll need to [create a DocumentDB account](/documentation/articles/documentdb-create-account/).
   ...
   If **DocumentDB (NoSQL)** is not visible in the Jumpbar, click **More Services** and then click **DocumentDB (NoSQL)**. If you don't have any accounts listed, you'll need to [create a DocumentDB account](/documentation/articles/documentdb-create-account/).
...
4. By default, **Storage Capacity** is set to **250 GB** to handle partitioned collections.
    ...
    Set storage capcity to **10 GB** if you want a [single partition collection](/documentation/articles/documentdb-partition-data/#single-partition-and-partitioned-collections/) with throughput levels from 400-10,000 request units/second (RU/s). One RU corresponds to the throughput of a read of a 1KB document. For more information about request units, see [Request units](/documentation/articles/documentdb-request-units/).

    Set storage capacity to **250 GB** if you want a [partitioned collection](/documentation/articles/documentdb-partition-data/#single-partition-and-partitioned-collections/) that can scale to handle an unlimited amount of storage over multiple partitions, and can have throughput levels starting at 2,500 RU/s.
...
A collection is a container of JSON documents and the associated JavaScript application logic. A collection is a billable entity, where the [cost](/documentation/articles/documentdb-performance-levels/) is determined by the provisioned throughput of the collection. Collections can span one or more partitions/servers and can scale to handle practically unlimited volumes of storage or throughput.
...
Collections do not have to be created using the portal, you can also create them using the [DocumentDB SDKs](/documentation/articles/documentdb-sdk-dotnet/) and the REST API.

- For a C# code sample, see the [C# collection samples](/documentation/articles/documentdb-dotnet-samples/#collection-examples/).
- For a Node.js code sample, see the [Node.js collection samples](/documentation/articles/documentdb-nodejs-samples/#collection-examples/).
- For a Python code sample, see [Python collection samples](/documentation/articles/documentdb-python-samples/#collection-examples/).
...
- Or you can add documents by using one of the [DocumentDB SDKs](/documentation/articles/documentdb-sdk-dotnet/). DocumentDB has .NET, Java, Python, Node.js, and JavaScript API SDKs. For C# code samples showing how to work with documents by using the DocumentDB .NET SDK, see the [C# document samples](/documentation/articles/documentdb-dotnet-samples/#document-examples/). For Node.js code samples showing how to work with documents by using the DocumentDB Node.js SDK, see the [Node.js document samples](/documentation/articles/documentdb-nodejs-samples/#document-examples/).
