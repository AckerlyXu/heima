...
description: Use Azure CLI 1.0 and Azure Resource Manager to manage regions in a DocumentDB database account. 
services: documentdb
author: dmakwana
manager: jhubbard
editor: ''
...
This article shows you how to add/remove a region in your DocumentDB account by using Azure CLI 1.0 commands and Azure Resource Manager templates. Region management can also be accomplished through the Azure Portal. Note that the commands in the following tutorial do not allow you to change failover priorities of the various regions. Only read regions can  be added or removed. The write region of a database account (failover priority of 0) cannot be added/removed.
...
To create DocumentDB, Table API, Graph API, or Mongo DB account using CLI 2.0, see [Create an Azure DocumentDB account using the Azure CLI](documentdb-automation-resource-manager-cli.md).
...
Most applications are built from a combination of different resource types (such as one or more DocumentDB accounts, storage accounts, a virtual network, or a content delivery network). The default Azure service management API and the Azure portal represented these items by using a service-by-service approach. This approach requires you to deploy and manage the individual services individually (or find other tools that do so), and not as a single logical unit of deployment.
...
## <a id="add-region-documentdb-account"></a>Task: Add Region to an DocumentDB account
...
### <a id="add-region-documentdb-account-cli"></a> Add a region to a DocumentDB account using Azure CLI 1.0 without Resource Manager templates
...
### <a id="add-region-documentdb-account-cli-arm"></a> Add a region to a DocumentDB account using Azure CLI 1.0 with Resource Manager templates
...
-	[Introduction to DocumentDB](./documentdb-resources.md)
