<properties
    pageTitle="Expire data in DocumentDB with time to live | Azure"
...
As soon as the document has expired (`ttl` + `_ts` >= current server time), the document is marked as "expired”. No operation will be allowed on these documents after this time and they will be excluded from the results of any queries performed. The documents are physically deleted in the system, and are deleted in the background opportunistically at a later time. This does not consume any [Request Units (RUs)](/documentation/articles/documentdb-request-units/) from the collection budget.
...
Yes. The collection must have [indexing policy set](/documentation/articles/documentdb-indexing-policies/) to either Consistent or Lazy. Trying to set DefaultTTL on a collection with indexing set to None will result in an error, as will trying to turn off indexing on a collection that has a DefaultTTL already set.

## Next steps
To learn more about Azure DocumentDB, refer to the service [*documentation*](/documentation/services/documentdb/) page.
