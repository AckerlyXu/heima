...
As Azure Cosmos DB is a multi-model database; it is important to note that this article is applicable to all data models and APIs in Azure Cosmos DB. This article uses generic terms such as *container* and an *item* to refer generically to a collection and a document, a node, or an entity, respectively.
...
2. To estimate the data storage requirements, enter the total number of items (e.g., documents) you expect to store.
...
## Use MongoDB API GetLastRequestStatistics command
...
The simplest way to get a good estimate of request unit charges for your MongoDB API database is to use the [Azure portal](https://portal.azure.cn) metrics. With the *Number of requests* and *Request Charge* charts, you can get an estimate of how many request units each operation is consuming and how many request units they consume relative to one another.
...
##  Exceeding reserved throughput limits in the MongoDB API
