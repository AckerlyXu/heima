---
title: Azure Cosmos DB diagnostic logging | Azure
...
Once you've started using one or more Azure Cosmos DB databases, you may want to monitor how and when your databases are accessed. Diagnostic logging in Azure Cosmos DB enables you to perform this monitoring. By enabling diagnostic logging, you can send logs to [Azure Storage](https://www.azure.cn/home/features/storage/), stream them to [Azure Event Hubs](https://www.azure.cn/home/features/event-hubs/), and/or export them to [Log Analytics](https://www.azure.cn/home/features/log-analytics/), which is part of [Operations Management Suite](https://www.microsoft.com/cloud-platform/operations-management-suite).
...
1. In the [Azure portal](https://portal.azure.cn), in your Azure Cosmos DB account, click **Diagnostic logs** in the left navigation, and then click **Turn on diagnostics**.
...
3. Click **Save**.
    ...
    If you receive an error that says "Failed to update diagnostics for \<workspace name>. The subscription \<subscription id> is not registered to use microsoft.insights." follow the [Troubleshoot Azure Diagnostics](/log-analytics/log-analytics-azure-storage) instructions to register the account, then retry this procedure.
...
For additional ease of management, in this tutorial we use the same resource group as the one that contains our Azure Cosmos DB database. Substitute values for ContosoResourceGroup, contosocosmosdblogs, and 'China North' for your own values, as applicable:
...
Before viewing your logs, you'll want to check and see if your Log Analytics workspace has been upgraded to use the new Log Analytics query language. To check this, open the [Azure portal](https://portal.azure.cn), click **Log Analytics** on the far left side, then select the workspace name as shown in the following image. The **OMS Workspace** page is displayed as shown in the following image.
...
| resourceTokenUserRid | resourceTokenUserRid_s | This is non-empty when [resource tokens](/cosmos-db/secure-access-to-data#resource-tokens) are used for authentication and points to resource ID of the user. |

## Next steps

- To gain an understanding of not only how to enable logging, but also the metrics and log categories supported by the various Azure services read both the [Overview of metrics in Azure](../monitoring-and-diagnostics/monitoring-overview-metrics.md) and [Overview of Azure Diagnostic Logs](../monitoring-and-diagnostics/monitoring-overview-of-diagnostic-logs.md) articles.
