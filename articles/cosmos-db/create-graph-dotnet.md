---
title: Build an Azure Cosmos DB .NET application using the Graph API | Azure
...
This quick start demonstrates how to create an Azure Cosmos DB account, database, and graph (container) using the Azure portal preview. You then build and run a console app built on the [Graph API](graph-sdk-dotnet.md) (preview).  
...
2. Once you've added some items, click the **Apply Filter** button, or right-click **Graph** and click **New Graph Query** to see the visual graph of your data. You can change how data is labeled and styled by clicking the **Style** button and changing your settings. Here's an example graph in Data Explorer, the labels, colors, and data shown can all be modified.
    ...
    ![Visual graph explorer in Data Explorer in the Azure portal preview](./media/create-graph-dotnet/azure-cosmos-db-graph-explorer.png)
...
1. In the [Azure portal preview](http://portal.azure.cn/), in your Azure Cosmos DB account, in the left navigation click **Keys**, and then click **Read-write Keys**. You'll use the copy buttons on the right side of the screen to copy the URI and Primary Key into the `App.config` file in the next step.
    ...
    ![View and copy an access key in the Azure portal preview, Keys blade](./media/create-graph-dotnet/keys.png)
