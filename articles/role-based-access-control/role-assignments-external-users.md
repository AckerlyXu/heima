...
In this example, the "Default tenant Azure" directory contains only users with the domain name "@alflanigan.partner.onmschina.cn".
...
However, granting **Owner** or **Contributor** access to an external user at the **Subscription** scope, does not allow the access to the admin user's directory, unless the **Global Admin** allows it. In the user proprieties,  the **User Type**, which has two common parameters, **Member** and **Guest** can be identified. A member is a user that is registered in the directory while a guest is a user invited to the directory from an external source. 
...
In PowerShell, use the [Get-AzureRmRoleDefinition](https://docs.microsoft.com/powershell/module/azurerm.resources/get-azurermroledefinition) command to export the **Reader** role in JSON format.
...
To create the custom role, you use the [New-AzureRmRoleDefinition](https://docs.microsoft.com/powershell/module/azurerm.resources/new-azurermroledefinition) command and provide the updated JSON role definition file.
...
![screenshot of custom role imported in the Azure portal](./media/role-assignments-external-users/18.png)

![screenshot of assigning custom imported role to user in the same directory](./media/role-assignments-external-users/19.png)

![screenshot of permissions for custom imported role](./media/role-assignments-external-users/20.png)

Users with this custom role can now create new support requests.

![screenshot of custom role creating support requests](./media/role-assignments-external-users/21.png)

Users with this custom role cannot perform other actions, such as create VMs or create resource groups.

![screenshot of custom role not able to create VMs](./media/role-assignments-external-users/22.png)

![screenshot of custom role not able to create new RGs](./media/role-assignments-external-users/23.png)
...
For this example, you can start with the built-in **Reader** role. To list the actions of the Reader role, use the [az role definition list](/cli/role/definition#az_role_definition_list) command.
...
To create the custom role, use the [az role definition create](/cli/role/definition#az_role_definition_create) command.
...
![Azure portal screenshot of custom role created using CLI 1.0](./media/role-assignments-external-users/26.png)
