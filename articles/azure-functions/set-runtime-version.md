...
1. In the [Azure portal](https://portal.azure.cn), navigate to the function app, and under **Configured Features**, choose **Function app settings**. 
...
1. In the [Azure portal](https://portal.azure.cn), navigate to your function app, and under **Configured Features**, choose **Application settings**.
...
## Target a version using Azure CLI
 ...
 You can also set the `FUNCTIONS_EXTENSION_VERSION` from the Azure CLI. Using the Azure CLI, update the application setting in the function app with the [az functionapp config appsettings set](https://docs.microsoft.com/cli/azure/functionapp/config/appsettings#set) command.
...
You can use the [Azure CLI locally](/cli/install-azure-cli) to execute this command after executing [az login](/cli/reference-index#az_login) to sign in.
