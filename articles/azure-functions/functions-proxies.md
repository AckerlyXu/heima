...
> [!NOTE] 
> Standard Functions billing applies to proxy executions. For more information, see [Azure Functions pricing](https://www.azure.cn/pricing/details/functions/).
...
For example, if a proxy has a route template, such as `/pets/{petId}`, the back-end URL can include the value of `{petId}`, as in `https://<AnotherApp>.chinacloudsites.cn/api/pets/{petId}`. If the route template terminates in a wildcard, such as `/api/{*restOfPath}`, the value `{restOfPath}` is a string representation of the remaining path segments from the incoming request.
...
You can also reference [application settings defined for the function app](/azure-functions/functions-how-to-use-azure-function-app-settings#develop) by surrounding the setting name with percent signs (%).
...
The proxies that you configure are stored in a *proxies.json* file, which is located in the root of a function app directory. You can manually edit this file and deploy it as part of your app when you use any of the [deployment methods](/azure-functions/functions-continuous-deployment) that Functions supports. 
...
[HTTP triggers]: /azure-functions/functions-bindings-http-webhook#http-trigger
