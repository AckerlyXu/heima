<properties
    pageTitle="Azure SQL Database Resource Limits | Azure"
...
Resources other than CPU, Memory, Log I/O, and Data I/O are enforced by denying new requests when limits are reached. When a database reaches the configured maximum size limit, inserts and updates that increase data size fail, while selects and deletes continue to work. Clients receive an [error message](/documentation/articles/sql-database-develop-error-messages/) depending on the limit that has been reached.
...
### Elastic pools
[Elastic pools](/documentation/articles/sql-database-elastic-pool/) share resources across databases in the pool. The following table describes the characteristics of Basic, Standard, and Premium elastic pools.
