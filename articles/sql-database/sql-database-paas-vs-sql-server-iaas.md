<properties
    pageTitle="SQL (PaaS) Database vs. SQL Server in the cloud on VMs (IaaS) | Azure"
...
* [Azure SQL Database](/home/features/sql-database/): A SQL database native to the cloud, also known as a platform as a service (PaaS) database or a database as a service (DBaaS) that is optimized for software-as-a-service (SaaS) app development. It offers compatibility with most SQL Server features.
...
**Azure SQL Database** is a relational database-as-a-service (DBaaS) hosted in the Azure cloud that falls into the industry categories of *Software-as-a-Service (SaaS)* and *Platform-as-a-Service (PaaS)*. [SQL database](/documentation/articles/sql-database-technical-overview/) is built on standardized hardware and software that is owned, hosted, and maintained by Microsoft. With SQL Database, you can develop directly on the service using built-in features and functionality. When using SQL Database, you pay-as-you-go with options to scale up or out for greater power with no interruption.
...
|  | Databases of up to 1 TB, or larger databases that can be [horizontally or vertically partitioned](/documentation/articles/sql-database-elastic-scale-introduction/#horizontal-and-vertical-scaling) using a scale-out pattern. |SQL Server instances with up to 64 TB of storage. The instance can support as many databases as needed. | |
|  | [Building Software-as-a-Service (SaaS) applications](/documentation/articles/sql-database-design-patterns-multi-tenancy-saas-applications/). |Migrating and building enterprise and hybrid applications. | |
|  | | |
...
|**Business continuity:**|In addition to built-in fault tolerance infrastructure capabilities, Azure SQL Database provides features, such as [automated backups](/documentation/articles/sql-database-automated-backups/), [Point-In-Time Restore](/documentation/articles/sql-database-recovery-using-backups/#point-in-time-restore), [Geo-Restore](/documentation/articles/sql-database-recovery-using-backups/#geo-restore), and [Active Geo-Replication](/documentation/articles/sql-database-active-geo-replication/) to increase business continuity. For more information, see [SQL Database business continuity overview](/documentation/articles/sql-database-business-continuity/).|SQL Server on Azure VMs lets you set up a high availability and disaster recovery solution for your database’s specific needs. Therefore, you can have a system that is highly optimized for your application. You can test and run failovers by yourself when needed. For more information, see [High Availability and Disaster Recovery for SQL Server on Azure Virtual Machines](/documentation/articles/virtual-machines-windows-sql-high-availability-dr/).|
...
|  | Supports [SQL Server transactional replication](https://msdn.microsoft.com/zh-cn/library/mt589530.aspx) as a subscriber to replicate data.|Fully supports [SQL Server transactional replication](https://msdn.microsoft.com/zh-cn/library/mt589530.aspx), [AlwaysOn Availability Groups](/documentation/articles/virtual-machines-windows-sql-high-availability-dr/), Integration Services, and Log Shipping to replicate data. Also, traditional SQL Server backups are fully supported|
|  | | |
...
**SQL Database** is sold to customers as a service, not with a license.  [SQL Server on Azure VMs](/documentation/articles/virtual-machines-windows-sql-server-iaas-overview/)  is sold with an included license that you pay per-minute. If you have an existing license, you can also use it.  

Currently, **SQL Database** is available in several service tiers, all of which are billed hourly at a fixed rate based on the service tier and performance level you choose. In addition, you are billed for outgoing Internet traffic at regular [data transfer rates](/pricing/details/data-transfers/). The Basic, Standard, and Premium service tiers are designed to deliver predictable performance with multiple performance levels to match your application’s peak requirements. You can change between service tiers and performance levels to match your application’s varied throughput needs. If your database has high transactional volume and needs to support many concurrent users, we recommend the Premium service tier. For the latest information on the current supported service tiers, see [Azure SQL Database Service Tiers](/documentation/articles/sql-database-service-tiers/). You can also create [elastic database pools](/documentation/articles/sql-database-elastic-pool/) to share performance resources among database instances.

With **SQL Database**, the database software is automatically configured, patched, and upgraded by Microsoft, which reduces your administration costs. In addition, its [built-in backup](/documentation/articles/sql-database-automated-backups/) capabilities help you achieve significant cost savings, especially when you have a large number of databases.
...
- [SQL Database Pricing](/pricing/details/sql-database/)
- [Virtual Machine Pricing](/pricing/details/virtual-machines/) for [SQL](/pricing/details/virtual-machines/#sql) and for [Windows](/pricing/details/virtual-machines/#windows)
- [Azure Pricing Calculator](/pricing/calculator/)

> [AZURE.NOTE] There is a small subset of features on SQL Server that are not applicable or not available with SQL Database. See [SQL Database Features](/documentation/articles/sql-database-features/) and [SQL Database Transact-SQL information](/documentation/articles/sql-database-transact-sql-information/) for more information. If you are moving an existing SQL Server solution to the cloud, see [Migrating a SQL Server database to Azure SQL Database](/documentation/articles/sql-database-cloud-migrate/). When you migrate an existing on-premises SQL Server application to SQL Database, consider updating the application to take advantage of the capabilities that cloud services offer. For example, you may consider using [Azure Web App Service](/home/features/web-site/) or [Azure Cloud Services](/home/features/cloud-services/) to host your application layer to increase cost benefits.
...
## Next steps
* See [Your first Azure SQL Database](/documentation/articles/sql-database-get-started/) to get started with SQL Database.
* See [SQL Database pricing](/pricing/details/sql-database/).
* See [Provision a SQL Server virtual machine in Azure](/documentation/articles/virtual-machines-windows-portal-sql-server-provision/) to get started with SQL Server on Azure VMs.
