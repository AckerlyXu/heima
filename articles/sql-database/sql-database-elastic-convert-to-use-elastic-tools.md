...
Easily manage your existing scaled-out sharded databases using Azure SQL Database database tools (such as the [Elastic Database client library](/documentation/articles/sql-database-elastic-database-client-library/)). You must first convert an existing set of databases to use the [shard map manager](/documentation/articles/sql-database-elastic-scale-shard-map-management/). 

## Overview
To migrate an existing sharded database: 

1. Prepare the [shard map manager database](/documentation/articles/sql-database-elastic-scale-shard-map-management/).
...
For more information about the ShardMapManager, see [Shard map management](/documentation/articles/sql-database-elastic-scale-shard-map-management/). For an overview of the elastic database tools, see [Elastic Database features overview](/documentation/articles/sql-database-elastic-scale-introduction/).
...
1. Single tenant per database (For terms, see the [glossary](/documentation/articles/sql-database-elastic-scale-glossary/).) 
...
## Summary
Once you have completed the setup, you can begin to use the Elastic Database client library. You can also use [data dependent routing](/documentation/articles/sql-database-elastic-scale-data-dependent-routing/) and [multi-shard query](/documentation/articles/sql-database-elastic-scale-multishard-querying/).
...
Use the split-merge tool to move data to or from a multi-tenant model to a single tenant model. See [Split merge tool](/documentation/articles/sql-database-elastic-scale-get-started/).

## Additional resources
For information on common data architecture patterns of multi-tenant software-as-a-service (SaaS) database applications, see [Design Patterns for Multi-tenant SaaS Applications with Azure SQL Database](/documentation/articles/sql-database-design-patterns-multi-tenancy-saas-applications/).
