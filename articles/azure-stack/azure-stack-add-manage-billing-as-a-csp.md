...
CSPSS subscriptions are associated with a shared-services tenant. When you register Azure Stack, you need to provide credentials for an account that is an owner of the subscription. The account you use to register Azure Stack can be different from the administrator account that you use for deployment; the two do *not* need to belong to the same domain. In other words, you may deploy using the tenant that you already use. For example you may use ContosoCSP.partner.onmschina.cn, then register using a different tenant, for example IURContosoCSP.partner.onmschina.cn. You will need to remember that you log in using ContosoCSP.partner.onmschina.cn when you do day-to-do Azure Stack administration. When you sign in to Azure using IURContosoCSP.partner.onmschina.cn when you need to do registration operations.
