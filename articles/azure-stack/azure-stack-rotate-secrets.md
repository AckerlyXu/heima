...
2.  Prepare a new set of replacement external certificates. The new set matches the certificate specifications outlined in the [Azure Stack PKI certificate requirements](/azure-stack/azure-stack-pki-certs).
...
1. Within the newly created **/Certificates** directory created in the Pre-steps, place the new set of replacement external certificates in the directory structure according to the format outlined in the Mandatory Certificates section of the [Azure Stack PKI certificate requirements](/azure-stack/azure-stack-pki-certs#mandatory-certificates).
2. Create a PowerShell Session with the [Privileged Endpoint](/azure-stack/azure-stack-privileged-endpoint) using the **CloudAdmin** account and store the sessions as a variable. You will use this variable as the parameter in the next step.
...
1. Create a PowerShell session with the [Privileged Endpoint](/azure-stack/azure-stack-privileged-endpoint).
