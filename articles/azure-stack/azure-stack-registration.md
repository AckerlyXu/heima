...
If not already installed, [install PowerShell for Azure Stack](/azure-stack/azure-stack-powershell-install). 
...
- Prompt you to log in as an owner of the Azure subscription to be used and set the `EnvironmentName` parameter to **AzureChinaCloud**.
...
|PrivilegedEndpoint|A pre-configured remote PowerShell console that provides you with capabilities like log collection and other post deployment tasks. To learn more, refer to the [using the privileged endpoint](/azure-stack/azure-stack-privileged-endpoint#access-the-privileged-endpoint) article.|
...
If you are registering Azure Stack in a disconnected environment (with no internet connectivity), you need to get a registration token from the Azure Stack environment and then use that token on a computer that can connect to Azure and has [PowerShell for Azure Stack installed](/azure-stack/azure-stack-powershell-install).  
