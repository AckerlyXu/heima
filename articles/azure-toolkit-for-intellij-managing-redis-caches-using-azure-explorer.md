...
1. When the **New Redis Cache** dialog box appears, specify the following options:
   ...
   a. **DNS Name**: Specifies the DNS subdomain for the new redis cache, which are prepended to ".redis.cache.chinacloudapi.cn"; for example: *wingtiptoys.redis.cache.chinacloudapi.cn*.
